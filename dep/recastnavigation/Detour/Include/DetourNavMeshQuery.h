...
public:
	...
	class dtNodePool* getNodePool() const { std::lock_guard<std::mutex> guard(m_navMeshQueryMutex); return m_nodePool; }

private:

	mutable std::mutex m_navMeshQueryMutex;
	...
	dtStatus closestPointOnPolyInternal(dtPolyRef ref, const float* pos, float* closest) const;
	dtStatus closestPointOnPolyBoundaryInternal(dtPolyRef ref, const float* pos, float* closest) const;
