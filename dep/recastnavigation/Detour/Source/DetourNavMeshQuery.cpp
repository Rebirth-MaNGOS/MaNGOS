...
{
	return closestPointOnPolyInternal(ref, pos, closest);
}


dtStatus dtNavMeshQuery::closestPointOnPolyInternal(dtPolyRef ref, const float* pos, float* closest) const
{
	dtAssert(m_nav);
...
{
	return closestPointOnPolyBoundaryInternal(ref, pos, closest);
}


dtStatus dtNavMeshQuery::closestPointOnPolyBoundaryInternal(dtPolyRef ref, const float* pos, float* closest) const
{
	dtAssert(m_nav);
										 ...
										 dtPolyRef* nearestRef, float* nearestPt) const
{
	...
	*nearestRef = 0;
	...
	for (int i = 0; i < polyCount; ++i)
	{
		...
		if (closestPointOnPolyInternal(ref, center, closestPtPoly) != DT_SUCCESS)
			continue;
										  ...
										  int* straightPathCount, const int maxStraightPath) const
{
	...
	*straightPathCount = 0;
	...
	if (closestPointOnPolyBoundaryInternal(path[0], startPos, closestStartPos) != DT_SUCCESS)
		return DT_FAILURE;
	...
	if (closestPointOnPolyBoundaryInternal(path[pathSize-1], endPos, closestEndPos) != DT_SUCCESS)
		return DT_FAILURE;
	...
	if (pathSize > 1)
	{
		...
		for (int i = 0; i < pathSize; ++i)
		{
			...
			if (i+1 < pathSize)
			{
				...
				{
					if (closestPointOnPolyBoundaryInternal(path[i], endPos, closestEndPos) != DT_SUCCESS)
						return DT_FAILURE;
