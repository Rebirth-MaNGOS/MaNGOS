...
struct MANGOS_DLL_DECL boss_firemawAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiShadowFlameTimer < uiDiff)
        {
			...
			// The spell has a 75 % chance to trigger after a Shadow Flame. (This is based on the sole comment found that said "This is a burst melee attack which often comes immediately after Shadow Flame.". http://www.allnightgamerz.com/firemaw.html)
			// Based on what was found here, http://z15.invisionfree.com/ExceptionGuild/ar/t253.htm, maybe the procc should be 30-35 % on each hit, but the only thing clear from the combat log posted was that it happened after the shadow flame.
			if (urand(0, 99) < 75)
