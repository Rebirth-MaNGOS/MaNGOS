...
struct MANGOS_DLL_DECL boss_mr_smiteAI : public ScriptedAI
{
    ...
    void PhaseEquipProcess()
    {
        if (m_creature->GetHealthPercent() < 33.0f)
        {
            ...
            SetEquipmentSlots(false, EQUIP_ID_HAMMER, EQUIP_UNEQUIP, EQUIP_UNEQUIP);
            DoCastSpellIfCan(m_creature, SPELL_SMITE_HAMMER);
        }
        else
            SetEquipmentSlots(false, EQUIP_ID_AXE, EQUIP_ID_AXE, EQUIP_UNEQUIP);
...
bool GOUse_go_defias_gunpowder(Player* /*pPlayer*/, GameObject *pGo)
{
    ...
    if (pInstance->GetData(TYPE_POWDER) == DONE)
        return false;

    pInstance->SetData(TYPE_POWDER, DONE);
    return false;
}
...
void AddSC_deadmines()
{
    ...
    newscript->pGOUse = &GOUse_go_defias_gunpowder;
    newscript->RegisterSelf();
}
