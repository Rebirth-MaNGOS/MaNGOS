...
struct MANGOS_DLL_DECL boss_jammalan_the_prophetAI : public ScriptedAI
{
    ...
    {
        if (m_pInstance)
            m_pInstance->SetData(TYPE_JAMMALAN_THE_PROPHET, NOT_STARTED);
    }
    ...
    {
        if (m_pInstance)
            m_pInstance->SetData(TYPE_JAMMALAN_THE_PROPHET, DONE);
    }
...
struct MANGOS_DLL_DECL boss_ogom_the_wretchedAI : public ScriptedAI
{
