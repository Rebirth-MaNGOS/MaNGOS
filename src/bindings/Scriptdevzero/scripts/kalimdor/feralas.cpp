...
enum
{
	...
	QUEST_ID_RETURN_TO_WITCH_DOCTOR_UZERI	= 3122,
};

struct MANGOS_DLL_DECL npc_witch_doctor_uzeriAI : public npc_escortAI
{
    npc_witch_doctor_uzeriAI(Creature* pCreature) : npc_escortAI(pCreature) { Reset(); }
	...
	void UpdateEscortAI(const uint32 uiDiff)
    {
		if (m_uiSpeechTimer && m_bOutro)							// handle RP at quest end
		{
            ...
            {		
                ...
                {
					case 1:
						m_creature->GetMotionMaster()->MovePoint(1, -4389.47f, 272.816f,25.4345f);
						m_uiSpeechTimer = 3000;
						break;
					case 2:
						m_creature->SetFacingTo(2.416f);
						m_creature->HandleEmote(EMOTE_ONESHOT_CHEER);
						m_creature->GenericTextEmote("Witch Doctor Uzer'i begins the ritual.", NULL, false);
						m_creature->SetSplineFlags(SPLINEFLAG_WALKMODE);
                        m_uiSpeechTimer = 5000;
						break;
					case 3:
						m_creature->GetMotionMaster()->MovePoint(2, -4391.54f, 274.713f,25.6319f);
						m_uiSpeechTimer = 1000;
						break;
					case 4:
						m_creature->SetFacingTo(5.50310f);
						m_uiSpeechTimer = 1000;
						break;
					case 5:
					...
					case 7:
						...
						break;
					case 9:		
						DoScriptText(UZERI_SAY, m_creature, pPlayer);
						m_uiSpeechTimer = 1000;
						break;
					case 10:						
...
CreatureAI* GetAI_npc_witch_doctor_uzeri(Creature* pCreature)
{
    return new npc_witch_doctor_uzeriAI(pCreature);
}

bool OnQuestRewarded_npc_witch_doctor_uzeri(Player* pPlayer, Creature* pCreature, Quest const* pQuest)
{
	if (pQuest->GetQuestId() == QUEST_ID_RETURN_TO_WITCH_DOCTOR_UZERI)
    {
		if (npc_witch_doctor_uzeriAI* pEscortAI = dynamic_cast<npc_witch_doctor_uzeriAI*>(pCreature->AI()))
...
void AddSC_feralas()
{
	...
	pNewscript = new Script;
    pNewscript->Name = "npc_witch_doctor_uzeri";
    pNewscript->GetAI = &GetAI_npc_witch_doctor_uzeri;
    pNewscript->pQuestRewardedNPC = &OnQuestRewarded_npc_witch_doctor_uzeri;
    pNewscript->RegisterSelf();
}
