...
struct MANGOS_DLL_DECL npc_spirit_guideAI : public ScriptedAI
{
    ...
    void SpellHitTarget (Unit* pUnit, const SpellEntry* pSpellEntry)
    {
		if (pUnit)
		{
			if (pSpellEntry->Id == 22012)
			{
				switch(pUnit->getClass())
				{
					case CLASS_HUNTER:
						pUnit->CastSpell(pUnit, 23498, false);
						pUnit->CastSpell(pUnit, 23499, false);

						break;
					case CLASS_WARLOCK:
					{
						Player* current_player = dynamic_cast<Player*>(pUnit);

						if (current_player)
						{
							uint32 spell;
							switch(current_player->m_warlockPetBeforeDeath)
							{
							case PET_IMP:
								spell = 23503;
								break;
							...
							if (spell)
								pUnit->CastSpell(pUnit, spell, false);
						}
