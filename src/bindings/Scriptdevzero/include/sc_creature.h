...
enum SelectTarget
{
	...
	SELECT_TARGET_SINGLE_ALLY,								//Any single non-self friendly
	SELECT_TARGET_ANY_ALLY									//All single non-self friendlies
};
...
struct MANGOS_DLL_DECL ScriptedAI : public CreatureAI
{
    ...
    virtual void OverrideAttackDistance(float& /*attackDistance*/, Unit*) {}
	...
	// Returns a list of unfriendly targets in range.
	std::list<Creature*> DoFindHostile(float range);
	...
	void MakeTauntImmune();

    // Returns unit with specified power
    Unit* SelectUnitWithPower(Powers power);

    // Returns a random player in the current map within max_distance from the creature. If max_distance is 0 there is no limitation.
    Player* GetRandomPlayerInCurrentMap(float max_distance = 0);

	// Returns a random player from the creature's threat list or NULL.
	Unit* GetRandomTargetFromThreatList();
...
struct PatAdd
{
	...
	explicit PatAdd(ObjectGuid n, double a, double b, double c) : guid(n), abstand(a), winkel(b), speed(c) {} 
};

struct MANGOS_DLL_DECL PatAI : public ScriptedAI
{
