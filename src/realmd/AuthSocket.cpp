...
// Includes used for the DDOS-protection.
#include <map>
#include <ctime>
#include <vector>
...
AuthSocket::~AuthSocket()
{
  ...
  // Anti-DDOS lists.
std::map<std::string, std::pair<time_t, int>> attempt_map;  // Store the address and the number of connections since the given time.
std::map<std::string, time_t> ban_map;                      // Store a list with banned clients.
time_t cleaning_time = 0;
...
void AuthSocket::OnAccept()
{
    ...
    {
        attempt_map[client_address] = std::pair<time_t, int>(current_time, 1); // If the client's not in the list, add it.
    }
    else
    {
        if (current_time - attempt_map[client_address].first < 60)  // Has it been more than 60 seconds since the last connection attempt?
        {
            if (attempt_map[client_address].second + 1 > 10) // Has the client made more than 10 attempts to connect in the last 60 secs?
            {
                ban_map[client_address] = current_time; // Df so; ban it.
				BASIC_LOG("Banning IP Address, to many connections in 60 seconds '%s' : ", client_address.c_str());
                attempt_map.erase(client_address);
                close_connection();
            }
            else
                attempt_map[client_address].second++; // Else count up towards the ban limit.
        }
        else
            attempt_map[client_address] = std::pair<time_t, int>(current_time, 1); // If there's been more than 60 secs reset the counter.
    }

    if (ban_map.count(client_address) != 0) // Is the current client banned?
    {
        if (current_time - ban_map[client_address] < 120) // If so, has it been more than two minutes?
            close_connection();                           // If not; close the connection.
        else
            ban_map.erase(client_address);                // If so; remove the ban.
    }

    if ( current_time - cleaning_time > 600 )           // Clean up the listst every 10 mins to keep memory usage down.
    {
        cleaning_time = current_time;

        std::map<std::string, std::pair<time_t, int>>::iterator itr = attempt_map.begin();
        while(itr != attempt_map.end())
        {
            if (current_time - (*itr).second.first > 120)
            {
                auto do_erase = itr;
                ++itr;
                attempt_map.erase(do_erase);
            }
            else
                ++itr;
        }

        std::map<std::string, time_t>::iterator b_itr = ban_map.begin();
        while(b_itr != ban_map.end())
        {
            if (current_time - (*b_itr).second > 120)
            {
                auto do_erase = b_itr;
                ++b_itr;
                ban_map.erase(do_erase);
            }
            else
                ++b_itr;
        }
...
void AuthSocket::OnRead()
{
    ...
    const size_t tableLength = sizeof(table) / sizeof(AuthHandler);
    ...
    while (1)
    {
        ...
        DEBUG_LOG("[Auth] Got command %u with status being %u.", _cmd, _status);
        ...
        for (i = 0; i < tableLength; ++i)
        {
            if (table[i].cmd == _cmd)
            {
                ...
                {
                    DEBUG_LOG("[Auth] Received unauthorized command %u", _cmd);
        ...
        if( result )
        {
            ...
            if (!locked)
            {
                ...
                else
                {
                    ...
                    // Remove the client from the attempt_table if the login is successfull.
                    attempt_map.erase(get_remote_address());
