	...
	FleeingMovementGenerator<T>::_setTargetLocation(T &owner)
{
	...
	if (MMAP::MMapFactory::IsPathfindingEnabled(owner.GetMap()->GetId()))
	{
	...
	FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
	...
	for(uint8 i = (uint8) urand(0, 17); i < 18; ++i)
	{
		...
		if (MMAP::MMapFactory::IsPathfindingEnabled(owner.GetMapId()))
		{
			...
			if (!mmap->GetNearestValidPosition(&owner, 3, 3, 5, targetX, targetY, targetZ, &polyRef))
				continue;
...
template<>
void
	FleeingMovementGenerator<Player>::_Init(Player &player)
{
	is_water_ok = true;
	is_land_ok  = true;


	player.RemoveMovementFlag(MOVEFLAG_WALK_MODE);
}
