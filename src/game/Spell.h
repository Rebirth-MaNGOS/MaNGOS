...
class Spell
{
	...
	/* Checks if the HOT is allowed to be applied.
	 * Returns true if the HOT is allowed to be casted
	 ...
	 * and overwrites any lesser blessings.
	 * Returns false otherwise. */
	bool CheckPaladinBlessingStacking(SpellEntry const* spellProto);
		...
		bool IsExceptedFromClientUpdate() const;
    ...
    protected:
		...
		int32 m_startedCasting;								// The time when the unit began to cast the spell
        int32 m_duration;
        ...
        struct TargetInfo
        {
			...
			PartialResistInfo partialResist:8;
			SpellMissInfo reflectResult:8;
			PartialResistInfo reflectPartialResist:8;
            uint8  effectMask:8;
            bool   processed:1;
        };
    ...
    struct MANGOS_DLL_DECL SpellNotifierCreatureAndPlayer
    {
        ...
        template<class T> inline void Visit(GridRefManager<T>  &m)
        {
            ...
            for(typename GridRefManager<T>::iterator itr = m.begin(); itr != m.end(); ++itr)
            {
                ...
                if ( (i_TargetType != SPELL_TARGETS_ALL && !itr->getSource()->isTargetableForAttack(i_spell.m_spellInfo->AttributesEx3 & SPELL_ATTR_EX3_CAST_ON_DEAD, i_TargetType == SPELL_TARGETS_AOE_DAMAGE))
                    // mostly phase check
                ...
                {
                    case PUSH_IN_FRONT:
                        if (i_castingObject->isInFront((Unit*)(itr->getSource()), i_radius, M_PI_F/2.0f ))
                            i_data->push_back(itr->getSource());
                        break;
                    case PUSH_IN_FRONT_LARGE:
                        if (i_castingObject->isInFront((Unit*)(itr->getSource()), i_radius, M_PI_F ))
                            i_data->push_back(itr->getSource());
                        break;
                    case PUSH_IN_FRONT_15:
                        ...
                        break;
                    case PUSH_IN_BACK:
                        if (i_castingObject->isInBack((Unit*)(itr->getSource()), i_radius, M_PI_F/3.0f ))
                            i_data->push_back(itr->getSource());
                        break;
                    case PUSH_SELF_CENTER:
