...
enum UnitState
{
    ...
    UNIT_STAT_LOST_CONTROL    = UNIT_STAT_FLEEING | UNIT_STAT_CONTROLLED | UNIT_STAT_CONFUSED,
...
enum TransportNudgeState
{
	NUDGESTATE_NONUDGE			= 0,
	NUDGESTATE_AWAITINGACCEPT	= 1,
	NUDGESTATE_ACTIVELYNUDGING	= 2
};
...
class MovementInfo
{
    public:
        MovementInfo() : moveFlags(MOVEFLAG_MOVE_STOP), time(0),
			...
			transportNudgeState(NUDGESTATE_NONUDGE), nextNudgeTime(0) {}            
        ...
        void ClearTransportData()
        {
            ...
            t_pos.o = 0.0f;
            t_time = 0;
			transportNudgeState = NUDGESTATE_NONUDGE;
			nextNudgeTime = 0;
        }
    ...
    //private:
		...
		TransportNudgeState transportNudgeState;
		uint32 nextNudgeTime;
};
...
class MANGOS_DLL_SPEC Unit : public WorldObject
{
    public:
        ...
        float GetMaxAttackDistance(const Unit *obj) const
        {
            float totalReach = this->GetFloatValue(UNIT_FIELD_COMBATREACH) + obj->GetFloatValue(UNIT_FIELD_COMBATREACH);

            float maxAttack = std::max(5.0f, totalReach);

            return maxAttack;
        }

        float GetPreferredAttackDistance(const Unit *obj) const
        {
            float max = this->GetMaxAttackDistance(obj);
			float zeroDistance = std::min(this->GetObjectBoundingRadius(),this->GetFloatValue(UNIT_FIELD_COMBATREACH))+std::min(obj->GetObjectBoundingRadius(),obj->GetFloatValue(UNIT_FIELD_COMBATREACH));
            
            return std::min(std::min(zeroDistance*1.5f, max*0.9f), max-0.5f);
        }

		void GetAttackPoint( const Unit *obj, float &x, float &y, float &z) const
		{
            //Get a good close distance for us to be from our target- best case scenario is our bounding volumes added together, but 
			//Sometimes our bounding volume is bigger than our reach, or real close, in that case, get just inside our combat reach 

            float pointDistance = GetPreferredAttackDistance(obj) - (obj->GetObjectBoundingRadius()+this->GetObjectBoundingRadius());

			this->GetNearPoint(obj,x,y,z,obj->GetObjectBoundingRadius(),pointDistance, GetAngle(obj));
		}
...
public:
		...
		Unit* SelectRandomAliveTarget(Unit* except = NULL, float radius = ATTACK_DISTANCE) const;
		Unit* SelectRandomTargetAliveOrDead(Unit* except = NULL, float radius = ATTACK_DISTANCE) const;
        ...
        void ProcDamageAndSpell(Unit *pVictim, uint32 procAttacker, uint32 procVictim, uint32 procEx, uint32 amount, time_t spellCastTime, WeaponAttackType attType = BASE_ATTACK, SpellEntry const *procSpell = NULL, bool isFirstTarget = true);
        ...
        SpellMissInfo MeleeSpellHitResult(Unit *pVictim, SpellEntry const *spell, int periodicCheckPvpBias);

        SpellMissInfo MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell, bool isPeriodicCheck=false, int periodicCheckPvpBias=0);
        SpellMissInfo SpellHitResult(Unit *pVictim, SpellEntry const *spell, PartialResistInfo &partialResist, bool canReflect = false, bool isPeriodicCheck=false, int periodicCheckPvpBias=0);
		SpellMissInfo MagicSpellBinaryResistResult(Unit *pVictim, SpellEntry const *spell);
		PartialResistInfo MagicSpellPartialResistResult(Unit *pVictim, SpellEntry const *spell,SpellSchoolMask forceToSchool=SPELL_SCHOOL_MASK_NONE);

        float GetUnitDodgeChance()    const;
        float GetUnitParryChance()    const;
        float GetUnitBlockChance()    const;
        float GetUnitCriticalChance(WeaponAttackType attackType, const Unit *pVictim, bool isSpell) const;

        virtual uint32 GetShieldBlockValue() const =0;
        uint32 GetUnitMeleeSkill(Unit const* target = NULL) const { return (target ? GetLevelForTarget(target) : getLevel()) * 5; }
        uint32 GetDefenseSkillValue(Unit const* target = NULL) const;
        uint32 GetWeaponSkillValue(WeaponAttackType attType, Unit const* target = NULL) const;
        float GetWeaponProcChance(WeaponAttackType attType) const;
		...
		{ 
			if (!HasAura(9991))			//Touch of Zanzil: Only negative stealth aura
				return HasAuraType(SPELL_AURA_MOD_STEALTH);
			else	
				return GetAurasByType(SPELL_AURA_MOD_STEALTH).size() > 1;
		}
        ...
        bool isTargetableForAttack(bool inversAlive = false, bool isAOE = false) const;
        bool isPassiveToHostile() { return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PASSIVE); }
		bool isAutoAttacking() { return hasUnitState(UNIT_STAT_MELEE_ATTACKING); }
        ...
        ObjectGuid const GetCharmerGuid() const { return GetGuidValuePtr(UNIT_FIELD_CHARMEDBY) ? GetGuidValue(UNIT_FIELD_CHARMEDBY) : ObjectGuid(); }
		...
		void InterruptSpellsTargettingUnit(bool withDelayed, ObjectGuid target);
        ...
        bool isVisibleForOrDetect(Unit const* u, WorldObject const* viewPoint, bool detect, bool inVisibleList = false, bool is3dDistance = true, bool skipDistanceCheck = false) const;
		...
		SpellAuraHolderList const& GetNegativeAuras() { return m_negativeAuras; };
		...
		SpellAuraProcResult HandleMayBreakOnDamage(Unit *pVictim, uint32 damage, Aura *triggeredByAura, SpellEntry const *procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown);
        SpellAuraProcResult HandlePCTModifier(Unit *pVictim, uint32 damage, Aura *triggeredByAura, SpellEntry const *procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown);
		SpellAuraProcResult HandleFlatModifier(Unit *pVictim, uint32 damage, Aura *triggeredByAura, SpellEntry const *procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown);
		...
		void SetPassiveToSpells(bool isPassive) { m_isPassiveToSpells = isPassive; }
		bool IsPassiveToSpells() { return m_isPassiveToSpells; }
		bool m_isPassiveToSpells;

        uint32 CalcArmorReducedDamage(Unit* pVictim, const uint32 damage);
		float GetTargetResistancePctVersusSpell(Unit *victim, SpellEntry const *entry,SpellSchoolMask forceToSchool=SPELL_SCHOOL_MASK_NONE);
        void CalculateDamageAbsorbAndResist(Unit *pCaster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, bool canReflect = false, PartialResistInfo partialResist=PARTIAL_RESIST_NONE);
		...
		TemporaryGameObject* SummonGameObject(uint32 entry, uint32 duration, float x, float y, float z, float ang, GOState initalState = GO_STATE_READY, uint32 animprogress = GO_ANIMPROGRESS_DEFAULT);
    ...
    protected:
		...
		SpellAuraHolderList m_negativeAuras;
