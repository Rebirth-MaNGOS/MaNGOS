...
pAuraProcHandler AuraProcHandler[TOTAL_AURAS]=
{
    ...
    &Unit::HandleMayBreakOnDamage,                          //  7 SPELL_AURA_MOD_FEAR
    ...
    &Unit::HandleMayBreakOnDamage,                          // 26 SPELL_AURA_MOD_ROOT
    ...
    &Unit::HandleMayBreakOnDamage,                          // 60 SPELL_AURA_MOD_PACIFY_SILENCE
    ...
    &Unit::HandleFlatModifier,                                  //107 SPELL_AURA_ADD_FLAT_MODIFIER
    &Unit::HandlePCTModifier,                                  //108 SPELL_AURA_ADD_PCT_MODIFIER
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch(hasteSpell->Id)
            {
                ...
                case 13877:
                {
					...
					float damage_reduction_to_armour = (float) CalcArmorReducedDamage(pVictim, damage) / (float) damage;
					basepoints0 = (uint32)((float) damage / (float) damage_reduction_to_armour); // Restore the original damage for the new victim to avoid reapplying damage reduction.
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            switch(auraSpellInfo->Id)
            {
				...
				case 21747:									// Lawbringer's 8/8 set bonus.
				{
					if (urand(0, 99) < 3)
					{
						CastSpell(this, 23544, true);
						return SPELL_AURA_PROC_OK;
					}
					else
						return SPELL_AURA_PROC_CANT_TRIGGER;
				}
				case 22438: // Mark of Detonation in BWL.
				{
					// Do not proc on spell cast.
					if (procSpell)
						return SPELL_AURA_PROC_FAILED;

					CastSpell(this, 22439, true);
    ...
    if (target == NULL)
	{
		if (procFlags & PROC_FLAG_SUCCESSFUL_POSITIVE_SPELL)
			target = pVictim;
		else
		{
			target = this;
			...
			{
				if (!IsPositiveTarget(triggerEntry->EffectImplicitTargetA[i],triggerEntry->EffectImplicitTargetB[i]))
				{
					target = pVictim;
					break;
				}
...
SpellAuraProcResult Unit::HandleProcTriggerDamageAuraProc(Unit *pVictim, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const* /*procSpell*/, uint32 /*procFlags*/, uint32 /*procEx*/, uint32 /*cooldown*/)
{
	...
	PartialResistInfo partialResist = this->MagicSpellPartialResistResult(pVictim,spellInfo);
    damageInfo.target->CalculateAbsorbResistBlock(this, &damageInfo, spellInfo,BASE_ATTACK,partialResist);
...
SpellAuraProcResult Unit::HandleMayBreakOnDamage(Unit *pVictim, uint32 damage, Aura *triggeredByAura, SpellEntry const* /*procSpell*/,uint32 /*procFlag*/, uint32 procEx, uint32 /*cooldown*/)
{
	Unit *target = triggeredByAura->GetTarget();

	//If we don't have a valid victim, just forget about it
	...
	{
		//Crits count for more!
		damageValue *= 2;
	}

	int spellLevel = 0;
	if (triggeredByAura->GetCaster() != NULL)
		spellLevel = triggeredByAura->GetCaster()->GetLevelForTarget(pVictim);
	else
		spellLevel = pVictim->getLevel();

	// The chance to dispel an aura depends on the damage taken with respect to the casters level.
    float max_dmg = spellLevel > 8 ? 25.0f * spellLevel - 150.0f : 50.0f;
    float chance = (damageValue/max_dmg) * 100.0f;
    if (roll_chance_f(chance))
	{
