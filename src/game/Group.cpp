...
uint32 Group::CanJoinBattleGroundQueue(BattleGroundTypeId bgTypeId, BattleGroundQueueTypeId bgQueueTypeId, uint32 MinPlayerCount, uint32 MaxPlayerCount)
{
    ...
    for(GroupReference *itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        // not in the same battleground level bracket, don't let join. A player below level 10 shouldn't be allowed to join either.
...
void Group::ResetInstances(InstanceResetMethod method, Player* SendMsgTo)
{
    ...
    for(BoundInstancesMap::iterator itr = m_boundInstances.begin(); itr != m_boundInstances.end();)
    {
        ...
        for (MemberSlot current_player : GetMemberSlots())
        {
            ...
            if (player && !player->IsBelowResetQuotaForInstance(entry->MapID))
            {
                ++itr;
                exit = true;
	...
	for (MemberSlot current_player : GetMemberSlots())
        ...
        for (MemberSlot current_player : GetMemberSlots())
        {
            // Move all offline members inside the instance to the entrance on reset.
            AreaTrigger const* at = sObjectMgr.GetMapEntranceTrigger(entry->MapID);
            CharacterDatabase.PExecute("UPDATE characters SET position_x = '%f', position_y = '%f', position_z = '%f', orientation = '%f', map = '%u' WHERE guid = '%u' AND map = '%u'",
...
static void RewardGroupAtKill_helper(Player* pGroupGuy, Unit* pVictim, uint32 count, bool PvP, float group_rate, uint32 sum_level, bool /*is_dungeon*/, Player* not_gray_member_with_max_level, Player* member_with_max_level, uint32 xp )
{
    ...
    if(!PvP)
    {
        ...
        // In Vanilla the group always gets full rep.
        // rewarded any alive/dead/near_corpse group member
        pGroupGuy->RewardReputation(pVictim, 1.f);
