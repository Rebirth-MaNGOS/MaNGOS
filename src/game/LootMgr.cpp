...
class LootTemplate::LootGroup                               // A set of loot definitions for items (refs are not allowed)
{
    public:
        ...
        bool HasQuestDrop(LootTemplateMap const &store, uint32 currentDepth) const;       // True if group includes at least 1 quest drop entry
        bool HasQuestDropForPlayer(LootTemplateMap const &store, Player const * player, uint32 currentDepth) const;
                                                            // The same for active quests of the player
        void Process(Player *player, Loot& loot, LootStore const& store, bool rate, uint32 currentDepth) const;                     // Rolls an item from the group (if any) and adds the item to the loot
    ...
    private:
        ...
        LootStoreItem const * Roll(Player *player) const;                 // Rolls an item from the group, returns NULL if all miss their chances
};
...
bool LootItem::AllowedForPlayer(Player const * player) const
{
    ...
    else
    {
		...
		// Quest 7810 is the arena master quest. The Arena Master trinket should always drop.
		if (pProto->StartQuest && pProto->StartQuest != 7810 && player->GetQuestStatus(pProto->StartQuest) != QUEST_STATUS_NONE && !player->HasQuestForItem(itemid))
            return false;
    }
...
bool Loot::FillLoot(uint32 loot_id, LootStore const& store, Player* loot_owner, bool personal, bool noEmptyError)
{
    ...
    tab->Process(loot_owner,*this, store,store.IsRatesAllowed ());     // Processing is done there, callback via Loot::AddItem()
...
LootStoreItem const * LootTemplate::LootGroup::Roll(Player *player) const
{
    if (!ExplicitlyChanced.empty())                         // First explicitly chanced entries are checked
    {
        ...
        {
            if (!sObjectMgr.IsPlayerMeetToCondition(player,ExplicitlyChanced[i].conditionId))
                continue;
    ...
    uint32 canLootCount = 0;

    for (uint32 i = 0; i < EqualChanced.size();++i)
    {
        if (sObjectMgr.IsPlayerMeetToCondition(player,EqualChanced[i].conditionId))
            canLootCount++;
    }
    ...
    uint32 rolledLoot = irand(0,canLootCount-1);
...
bool LootTemplate::LootGroup::HasQuestDrop(const LootTemplateMap &store, uint32 currentDepth) const
{
    for (LootStoreItemList::const_iterator i=ExplicitlyChanced.begin(); i != ExplicitlyChanced.end(); ++i)
    {
        if (i->needs_quest)
            return true;
        else if (i->mincountOrRef < 0)
        {
            ...
            if (Referenced->HasQuestDrop(store,currentDepth+1))
                return true;
        }
    }
...
bool LootTemplate::LootGroup::HasQuestDropForPlayer(const LootTemplateMap &store, Player const * player, uint32 currentDepth) const
{
    for (LootStoreItemList::const_iterator i=ExplicitlyChanced.begin(); i != ExplicitlyChanced.end(); ++i)
    {
        ...
        else if (i->mincountOrRef < 0)
        {
            ...
            if (Referenced->HasQuestDropForPlayer(store, player, currentDepth+1))
                return true;
        }
    }
...
void LootTemplate::LootGroup::Process(Player *player, Loot& loot, LootStore const& store, bool rate, uint32 currentDepth) const
{
    LootStoreItem const * item = Roll(player);
    if (item != NULL)
    {
        if (item->mincountOrRef < 0)                           // References processing
        {
            ...
            for (uint32 loop=0; loop < item->maxcount; ++loop )// Ref multiplicator
                Referenced->Process(player, loot, store, rate, currentDepth+1);
        }
        else                                                // Plain entries
...
void LootTemplate::Process(Player *player, Loot& loot, LootStore const& store, bool rate, uint32 currentDepth, uint8 groupId) const
{
    //This is to protect against templates that loop causing a stack overflow
    if (currentDepth >= 10)
        return;
    ...
    if (groupId)                                            // Group reference uses own processing of the group
    {
        ...
        Groups[groupId-1].Process(player, loot,store,rate,currentDepth+1);
        return;
    }
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin() ; i != Entries.end() ; ++i )
    {
        ...
        if (i->mincountOrRef < 0)                           // References processing
        {
            ...
            for (uint32 loop=0; loop < i->maxcount; ++loop )// Ref multiplicator
                Referenced->Process(player, loot, store, rate, currentDepth+1, 0);
        }
    ...
    for (LootGroups::const_iterator i = Groups.begin( ) ; i != Groups.end( ) ; ++i )
        i->Process(player, loot, store, rate, currentDepth+1);
}

// True if template includes at least 1 quest drop entry
bool LootTemplate::HasQuestDrop(LootTemplateMap const& store, uint32 currentDepth, uint8 groupId) const
{
    //Protection from stack overflow, in case we have a loop in the loot tables
    if (currentDepth >= 10)
        return false;
    ...
    if (groupId)                                            // Group reference
    {
        ...
        return Groups[groupId-1].HasQuestDrop(store, currentDepth+1);
    }
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i )
    {
        if (i->mincountOrRef < 0)                           // References
        {
            ...
            if (Referenced->second->HasQuestDrop(store, currentDepth+1) )
                return true;
        }
    ...
    // Now processing groups
    for (LootGroups::const_iterator i = Groups.begin() ; i != Groups.end() ; ++i )
        if (i->HasQuestDrop(store, currentDepth+1))
            return true;
...
bool LootTemplate::HasQuestDropForPlayer(LootTemplateMap const& store, Player const* player, uint32 currentDepth, uint8 groupId) const
{
    //Protection from stack overflow, in case we have a loop in the loot tables
    if (currentDepth >= 10)
        return false;
    ...
    if (groupId)                                            // Group reference
    {
        if (groupId > Groups.size())
            return false;                                   // Error message already printed at loading stage
        return Groups[groupId-1].HasQuestDropForPlayer(store, player, currentDepth+1);
    }
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin() ; i != Entries.end() ; ++i )
    {
        if (i->mincountOrRef < 0)                           // References processing
        {
            ...
            if (Referenced == store.end() )
                continue;                                   // Error message already printed at loading stage
            if (Referenced->second->HasQuestDropForPlayer(store, player, currentDepth+1) )
                return true;
        }
    ...
    // Now checking groups
    for (LootGroups::const_iterator i = Groups.begin(); i != Groups.end(); ++i )
        if (i->HasQuestDropForPlayer(store, player, currentDepth+1))
            return true;
