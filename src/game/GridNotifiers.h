...
namespace MaNGOS
{
    ...
    {
        Unit const* i_skipped_receiver;
        PackedGuid i_mover_guid;
        MovementInfo i_movement_info;
        WorldPacket* i_received_movement;
        uint32 i_initial_timestamp;

        MovementOpcodeDeliverer(Unit const* skipped, PackedGuid guid, MovementInfo info, WorldPacket* received_movement)
    ...
    */
	...
	class AnyHostileUnitInObjectRangeCheck
    {
        public:
            ...
            bool operator()(Unit* u)
            {
                if(u->isAlive() && i_obj->IsWithinDistInMap(u, i_range) && i_funit->IsHostileTo(u))
                    return true;
                else
                    return false;
            }
	...
	class AnyUnitAliveOrDeadInObjectRangeCheck
    {
        public:
            AnyUnitAliveOrDeadInObjectRangeCheck(WorldObject const* obj, float range) : i_obj(obj), i_range(range) {}
            WorldObject const& GetFocusObject() const { return *i_obj; }
            bool operator()(Unit* u)
            {
	...
	class CallOfHelpWhileChasingOrFearedInRangeDo
    {
        public:
            CallOfHelpWhileChasingOrFearedInRangeDo(Unit* funit, Unit* enemy, float range)
                : i_funit(funit), i_enemy(enemy), i_range(range)
            {}
            void operator()(Creature* u)
            {
				...
				// Do not aggro mobs already chasing someone.
				if (u->GetMotionMaster()->GetCurrentMovementGeneratorType() == MovementGeneratorType::CHASE_MOTION_TYPE)
					return;

				// Do not call an evading friend.
				if (u->IsInEvadeMode())
					return;

				// Only assist if there is no cd currently applied.
				if (u->GetFriendAssistTimeout() != 0)
					return;
				else
					u->SetFriendAssistTimeout(10000);
    ...
    class AnyStealthedCheck
    {
        public:
			...
			bool operator()(GameObject *go) { return go->GetGOInfo()->IsStealthed(); }
        private:
            WorldObject const* i_fobj;
    };

	class TrapDetectionCheck
	{
	public:
		explicit TrapDetectionCheck(WorldObject const *focus, int detectSkill, float radius, bool ignoreFriendly=false, Unit const *owner=NULL) : m_focus(focus), m_owner(owner), m_detectSkill(detectSkill), m_radius(radius), m_ignoreFriendly(ignoreFriendly) {}
		WorldObject const& GetFocusObject() const { return *m_focus; }
		Unit const& GetOwner() const { return *m_owner; }
		float GetRadius() const { return m_radius; }

		//If we're looking for units we're looking for any units who aren't friends with the owner - if there is no owner
		//then I guess something terrible has happened.
		bool operator()(Creature *c)
		{
			if (m_owner == NULL)
			{
				//I guess let all non-players through since only hunter traps and other
				//factioned traps should trigger on non-players
				return false;
			}

			if (c->IsTotem())
			{
				// Totems cannot trigger traps.
				return false;
			}

			//Let anyone through who's friendly
			if (!c->isAlive() || c->IsFriendlyTo(m_owner))
				return false;

			//Get the detection radius
			float radius = GetDetectRadius(c);

			return m_focus->IsWithinDistInMap(c,radius);
		}
		...
		bool operator()(Player *p)
		{
			...
			if (m_ignoreFriendly && m_owner != NULL && p->IsFriendlyTo(m_owner))
				return false;

			//Get the detection radius
			float radius = GetDetectRadius(p);

			return m_focus->IsWithinDistInMap(p,radius);
		}
	protected:
		float GetDetectRadius(Unit *u)
		{
			float radius = m_radius;

			//Only include detect skill when we're stealth-affected
			if (m_detectSkill >= 0 && u->GetVisibility()==VISIBILITY_GROUP_STEALTH)
			{
				//Calculate a smaller radius based on the unit's stealth vs. our detect
				//We should have about 50% radius versus same-level stealthed mobs
				int32 stealthLevel = u->GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_STEALTH_LEVEL,0);
				int32 stealthSkill = 0;
				if (u->GetTypeId() == TYPEID_PLAYER)
				{
					stealthSkill = u->GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_STEALTH,0);
				} else
				{
					//Mobs don't have a stealth skill and instead use their level * 5
					stealthSkill = (u->GetLevelForTarget(m_owner) * 5);
				}

				stealthLevel += stealthSkill;

				float radiusMod = std::max(std::min((0.01f * (float)(m_detectSkill - stealthLevel)),0.5f),-0.25f);

				radius = (0.5f + radiusMod) * m_radius;
			}
