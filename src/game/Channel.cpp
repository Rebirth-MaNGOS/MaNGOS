...
{
	m_announce = false;                                 // override:  no join/leave announces since below code for built-in channels does not function
...
void Channel::Join(ObjectGuid p, const char *pass)
{
    ...
    if (IsOn(p))
    {
        ...
        {
			/* Causing Spam in Guild Recruitment forum 
...
//        bool changeowner = m_players[p].IsOwner();
        ...
        /* Commenting out ownership change reverb to channel.  This should only apply to custom channels not all.
...
void Channel::SetOwner(ObjectGuid guid, bool exclaim)
{
    ...
    if (m_ownerGuid)
    {
		...
		Player* p_owner = sObjectMgr.GetPlayer(m_ownerGuid);
		if (p_owner && !p_owner->isGameMaster())
		{
			WorldPacket data;
