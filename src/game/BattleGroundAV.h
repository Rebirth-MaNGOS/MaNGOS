...
enum BG_AV_Beacons
{
    BG_AV_BEACON_GUSE			= 178545,
    BG_AV_BEACON_ICHMAN			= 178726,
    BG_AV_BEACON_VIPORE			= 178724,
    BG_AV_BEACON_SLIDORE		= 178725,
    BG_AV_BEACON_MULVERICK		= 178546,
    BG_AV_BEACON_JEZTOR			= 178547,

	BG_AV_BEACON_BIRD_GUSE		= 800012,
	BG_AV_BEACON_BIRD_ICHMAN	= 13161,
	BG_AV_BEACON_BIRD_VIPORE	= 800010,
	BG_AV_BEACON_BIRD_SLIDORE	= 800009,
	BG_AV_BEACON_BIRD_MULVERICK = 13178,
	BG_AV_BEACON_BIRD_JEZTOR	= 800011
};

enum BG_AV_BeaconEvents
{
    BG_AV_EVENT_SPAWN_GUSE_BEACON		= 7143,
    BG_AV_EVENT_SPAWN_ICHMAN_BEACON		= 7319,
    BG_AV_EVENT_SPAWN_VIPORE_BEACON		= 7327,
    BG_AV_EVENT_SPAWN_SLIDORE_BEACON		= 7323,
    BG_AV_EVENT_SPAWN_MULVERICK_BEACON		= 7158,
    BG_AV_EVENT_SPAWN_JEZTOR_BEACON		= 7150
};
...
#define BG_AV_AIRSTRIKES 65
#define BG_AV_AIRSTRIKES_MAX 6
...
const uint32 BG_AV_reset_quests[] =
{
    // Horde
...
public:
    ...
    virtual void GetInsigniaLoot(Loot *loot, Player *victim, Player *looter);
    ...
    /* Return the number of turn-ins for the commanders.
       Slidore & Guse = 1
       Vipore & Jeztor = 2
       Ichman & Mulverick = 3*/
    uint32 GetWingCommanderQuestStatus(BattleGroundAVTeamIndex team, uint32 commander) {
        return m_Team_QuestStatus[team][commander];
    }

    void SetAirstrikeLaunched(uint8 airstrike) {
        m_bAirstrikeLaunched[airstrike] = true;
    }
    void HandleBeaconEvent(uint32 event_id);

    std::vector<uint32> m_vecBeaconEventQueue;
    std::map<uint32, uint32> m_BeaconTimerMap;
...
private:
    ...
    void OnObjectDBLoad(GameObject* object);
    ...
    bool m_bAirstrikeLaunched[BG_AV_AIRSTRIKES_MAX];

    std::map<uint32, ObjectGuid> m_GameObjectStorageMap;
	std::vector<GameObject*> m_mineSupplies[2];
};
