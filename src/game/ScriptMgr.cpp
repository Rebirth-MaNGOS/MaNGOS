...
uint32 ScriptMgr::GetVendorId(Player *pPlayer, const Creature *pCreature) const
{
	if (!m_pGetNPCVendorId)
		return 0;

	return m_pGetNPCVendorId(pPlayer, pCreature);
}
...
bool ScriptMgr::OnMobGameObjectUse(Creature* pMob, GameObject* pGameObject)
{
	return m_pOnMobGOUse != NULL && m_pOnMobGOUse(pMob, pGameObject);
}
...
DatabaseType* ScriptMgr::GetScriptdevDB()
{
	return m_pGetScriptdevDB();
}
...
#   define GET_SCRIPT_HOOK_PTR(P,N)             \
	...
	GET_SCRIPT_HOOK_PTR(m_pGetNPCVendorId,			   "GetNPCVendorId");
	...
	GET_SCRIPT_HOOK_PTR(m_pOnMobGOUse,				   "GOMobUse");
	...
	GET_SCRIPT_HOOK_PTR(m_pGetScriptdevDB,			   "GetScriptdevDB");
