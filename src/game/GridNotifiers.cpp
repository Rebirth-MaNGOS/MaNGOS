...
void MovementOpcodeDeliverer::Visit(CameraMapType& m)
{
    for (auto iter = m.begin(); iter != m.end(); ++iter)
    {
            ...
            if (WorldSession* session = owner->GetSession())
            {
                i_movement_info.UpdateTime(i_initial_timestamp + 175 + session->GetLatency());
...
void MaNGOS::RespawnDo::operator()( Creature* u ) const
{
    ...
    if (map->IsBattleGround())
    {
        ...
        if (!((BattleGroundMap*)map)->GetBG() || !((BattleGroundMap*)map)->GetBG()->IsActiveEvent(eventId.event1, eventId.event2))
            return;
    }
