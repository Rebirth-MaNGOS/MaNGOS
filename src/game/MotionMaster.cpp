...
{
    if (m_owner->hasUnitState(UNIT_STAT_CAN_NOT_MOVE) || m_owner->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) || m_movementSuspended)
        return;
...
void MotionMaster::SuspendChaseMovement()
{
	if (GetCurrentMovementGeneratorType() == MovementGeneratorType::CHASE_MOTION_TYPE)
	{
		...
		m_movementSuspended = true;
	}
}

void MotionMaster::ResumeChaseMovement()
{
	if (m_movementSuspended)
	{
		if (m_owner->getVictim())
        {
			MoveChase(m_owner->getVictim(), 0.f);
        }
