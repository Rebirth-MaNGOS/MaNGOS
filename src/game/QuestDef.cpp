    ...
    if( pPlayer )
    {
        if( RewMoneyMaxLevel > 0 )
        {
            ...
            else if (qLevel <= 60)
			{
				...
				if (qLevel == 0 && pLevel < 60)
				{
					//quests of QLevel 0 should scale XP rewards to level
					//Do this by dividing the fullXP by XP from 60 to 61, and then multiply it by XP from player's CL to next CL
					int expRequireToLevel = sObjectMgr.GetXPForLevel(pPlayer->getLevel());
					int expFrom60To61 = 217350; // ( (8*60) + (5 * 60-30) ) * (45 + 5 * 60) - http://www.wowwiki.com/index.php?title=Formulas:XP_To_Level&oldid=303779

					fullxp = (fullxp / (float)expFrom60To61) * expRequireToLevel;

					qLevel = pLevel;  //Don't apply XP reductions based on quest/player level difference
				}
			}
