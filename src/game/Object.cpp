...
void WorldObject::GenericTextEmote(const char *text, Unit *target, bool IsBossEmote)
{
    WorldPacket data(SMSG_MESSAGECHAT, 200);
    BuildMonsterChat(&data, GetObjectGuid(), CHAT_MSG_TEXT_EMOTE, text, LANG_UNIVERSAL,
...
void WorldObject::MonsterEmoteToZone(int32 textId, uint32 language, Unit* target)
{
    MaNGOS::MonsterChatBuilder say_build(*this, CHAT_MSG_TEXT_EMOTE, textId, language, target);
    MaNGOS::LocalizedPacketDo<MaNGOS::MonsterChatBuilder> say_do(say_build);
...
void WorldObject::BuildMonsterChat(WorldPacket *data, ObjectGuid senderGuid, uint8 msgtype, char const* text, uint32 language, char const* name, ObjectGuid targetGuid, char const* targetName)
{
	...
	*data << ObjectGuid(targetGuid);                        // Unit Target
	if (msgtype != CHAT_MSG_TEXT_EMOTE && targetGuid && !targetGuid.IsPlayer())
	{
