...
CreatureEventAI::CreatureEventAI(Creature *c ) : CreatureAI(c)
{
	...
	if ((cInfo->rank == 2 || cInfo->rank == 4) && m_creature->GetMapId() < 2 && urand(0,100) > 90)
...
void CreatureEventAI::ProcessAction(CreatureEventAI_Action const& action, uint32 rnd, uint32 EventId, Unit* pActionInvoker)
{
    switch (action.type)
    {
		...
		{
			bool LoS_check = false;
			// ignore no affect case unless state = 0 and target is not in line of sight
			if (m_creature->getVictim() && !m_CombatMovementEnabled && action.combat_movement.state == 0 && !m_creature->IsWithinLOSInMap(m_creature->getVictim()))
				LoS_check = true;
            else if(m_CombatMovementEnabled==(action.combat_movement.state!=0))
                return;	
            ...
            {
				//Forced combat movement because of missing line of sight
				if(LoS_check)
				{
					m_CombatMovementEnabled = true;
					if (m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() == IDLE_MOTION_TYPE)
		...
		case ACTION_T_IMMUNITY:
		{
			m_creature->ApplySpellImmune(0, IMMUNITY_DAMAGE, action.immunity.school, action.immunity.apply);
			break;
		}
	default:
		break;
...
void CreatureEventAI::ResetToHome()
{
...
void CreatureEventAI::MoveInLineOfSight(Unit *who)
{
    ...
    if (m_creature->CanInitiateAttack() && who->isTargetableForAttack() &&
        m_creature->IsHostileTo(who) && (who->isInAccessablePlaceFor(m_creature) || m_creature->GetEntry() == 115184)) // Guards in Cenarion Hold, entry 115184, should ignore all LOS and the like.
    {
        if (!m_creature->CanFly() && m_creature->GetDistanceZ(who) > CREATURE_Z_ATTACK_RANGE && m_creature->GetEntry() != 115184)
            return;

        float attackRadius = m_creature->GetAttackDistance(who);
        if (m_creature->IsWithinDistInMap(who, attackRadius) && (m_creature->IsWithinLOSInMap(who) || m_creature->GetEntry() == 115184))
        {
...
{
	if (m_creature->GetEntry() == 115184) // The guards in Cenarion Hold see all! :D
		return true;
