...
{ 
	...
	// If no spell chain is found for the teaching spell try looking for a spell chain for the spell you learn.
	if (!chain_node)
		chain_node = sSpellMgr.GetSpellChainNode(triggerSpell);
...
void WorldSession::HandleUnstablePet( WorldPacket & recv_data )
{
	...
	// Make sure the player doesn't have a dead pet that has despawned.
	{
        QueryResult *result = CharacterDatabase.PQuery("SELECT entry FROM character_pet WHERE owner = '%u' AND (slot ='%u' OR slot = '%u')",
            _player->GetGUIDLow(), 100, 0);
        if(result)
        {
            Field *fields = result->Fetch();
            creature_id = fields[0].GetUInt32();
			...
			SendStableResult(StableResultCode::STABLE_ERR_STABLE);
			return;
        }
    }
...
void WorldSession::HandleStableSwapPet( WorldPacket & recv_data )
{
	...
	// Do not overwrite dead pets. Return instead.
