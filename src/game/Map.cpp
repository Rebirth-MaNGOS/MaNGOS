...
//#define DEBUG_LOAD_CONTINENTS		//  Define to load all grids for the continents on server start.
...
Map::Add(T *obj)
{
    ...
    // A type check to see if we have a GameObject.
    GameObject* pObj = dynamic_cast<GameObject*>(obj);

    // If it's a GameObject be sure to respect the respawn time and not respawn it until the time runs out.
    if(pObj)
    {
        if(pObj->GetRespawnTime() <= time(NULL) || pObj->GetSpellId() != 0)
        {
            obj->AddToWorld();
        }
    }
...
void Map::ExtendDeadCreatureRespawn(uint32 lowGuid, time_t timeVal)
{
    this->GetPersistentState()->ExtendCreatureRespawnTime(lowGuid,timeVal);
}
...
bool DungeonMap::Add(Player *player)
{
    ...
    else
    {
        ...
        if (pGroup)
        {
            ...
            if (playerBind && player->IsBelowResetQuotaForInstance(GetId()))
            {
            ...
            else
            {
                ...
                if (groupBind->state != GetPersistentState() && player->IsBelowResetQuotaForInstance(GetId()))
                {
...
bool DungeonMap::Reset(InstanceResetMethod method)
{
    ...
    if(HavePlayers())
    {
        ...
        else
        {
            ...
            // Do not reset the map for a raid since it'll delete the map.
            if (!IsRaid())
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld.GetGameTime()))
    {
        ...
        case SCRIPT_COMMAND_QUEST_EXPLORED:
        {
            ...
            if ((worldObject->GetTypeId() != TYPEID_UNIT || ((Unit*)worldObject)->isAlive()) &&
                ...
                player->GroupEventHappens(step.script->questExplored.questId, player);
            else
...
{
    Creature *creature = m_objectsStore.find<Creature>(guid, (Creature*)NULL);
...
{
    Pet *pet = m_objectsStore.find<Pet>(guid, (Pet*)NULL);
...
{
    GameObject* object = m_objectsStore.find<GameObject>(guid, (GameObject*)NULL);
...
{
    DynamicObject* object = m_objectsStore.find<DynamicObject>(guid, (DynamicObject*)NULL);
