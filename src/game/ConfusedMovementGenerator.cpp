...
bool findValidMovemapsLocation(const dtNavMeshQuery* /*query*/, bool /*isWaterOkay*/, bool /*isLandOkay*/, float /*startX*/, float /*startY*/, float /*startZ*/, float& /*endX*/, float& /*endY*/, float& /*endZ*/, TerrainInfo* /*map*/)
{
    //MaNGOS::NormalizeMapCoord(endX);
    //MaNGOS::NormalizeMapCoord(endY);

    //bool is_water = map->IsInWater(endX,endY,endZ);
    //// if generated wrong path just ignore
    //if ((is_water && !isWaterOkay) || (!is_water && !isLandOkay))
    //{
    //    return false;
    //}
    ...
    //if (isWaterOkay)
    //    filter.setIncludeFlags(NAV_MAGMA|NAV_SLIME|NAV_WATER);

    //float startPoint[3] = {y, z, x};
    //float extents[3] = {3, 5, 3 };
    //dtPolyRef polyRef;
    //float outStartPoint[3] = {0,0,0};
    //
    ////Use movemaps to do confuse path - start by finding the closest point on the navmesh to where we are now
    //    if (query->findNearestPoly(startPoint, extents, &filter,&polyRef,outStartPoint) != DT_SUCCESS)
    //    {
    //    }
    return true;
}
...
bool ConfusedMovementGenerator<T>::Update(T &unit, const uint32 &diff)
{
    ...
    if (i_nextMoveTime.Passed())
    {
        ...
        {
            if (!MovementGenerator::IsActive(unit))   // force stop processing (movement can move out active zone with cleanup movegens list)
