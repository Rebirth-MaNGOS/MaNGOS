...
{
    SpellModifier() : charges(0), laterDeletion(false), lastAffected(NULL) {}

    SpellModifier(SpellModOp _op, SpellModType _type, int32 _value, uint32 _spellId, uint64 _mask, int16 _charges = 0)
        : op(_op), type(_type), charges(_charges), value(_value), mask(_mask), spellId(_spellId), laterDeletion(false), lastAffected(NULL)
    {}
    ...
    uint32 spellId;
	bool laterDeletion;			// Used for marking mods for deletion upon a spell being successfully cast.
...
enum PlayerLoginQueryIndex
{
    ...
    PLAYER_LOGIN_QUERY_LOADINSTANCEQUOTA,
...
class MANGOS_DLL_SPEC Player : public Unit
{
    ...
    public:
		...
		int32 m_loginDelayTimer;		// Custom timer, used to prevent player from moving while logging in
        ...
        Team m_ChatTeam; // Used to keep track of which faction's chat a gm is in.
		...
		bool ValidateInventory(); // Used to validate the player's inventory, kicking as well as clearing the player's item update queue if an error occurs.
        ...
        { 
            // Clear combo points when switching target.
            if (guid != ObjectGuid() && m_curSelectionGuid != ObjectGuid())
                ClearComboPoints(); 
...
public:
	...
	void AddToResetQuotaList(uint32 instanceMapId) { m_InstanceResetLists[instanceMapId].push_back(time(0)); }
		...
		WarlockPet m_warlockPetBeforeDeath;
		void SetWarlockPetBeforeDeath();
		...
		TransportNudgeState GetNudgeState() const { return m_movementInfo.transportNudgeState; }
		...
		void HandleStealthedTrapsDetection();
		...
		void SupercedeSpell(uint32 old_spell_id, uint32 new_spell_id);
	...
	void _LoadInstanceQuota(QueryResult *result);
    ...
    public:
        ...
        // AI for the Gnomish Mind Control Cap
        void CreateAI();
        void DeleteAI();
        PetAI* AI() { return i_AI ? i_AI : NULL; }

        void SetCharmedCDTimer(uint32 mSec);
        uint32 GetCharmedCDTimer();
    private:
        PetAI* i_AI;
        uint32 charmedCooldownTimer;
