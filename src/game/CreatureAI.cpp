...
{
	if (m_creature->GetCreatureInfo()->type==CREATURE_TYPE_CRITTER)
	{
		if (!m_creature->IsPet())
		{
...
CanCastResult CreatureAI::CanCastSpell(Unit* pTarget, const SpellEntry *pSpell, bool isTriggered)
{
    ...
    if (!isTriggered)
    {
		...
		if (pSpell->School && m_creature->ProhibitTimer[pSpell->School])
			return CAST_FAIL_STATE;
...
{
	// Check to make sure we arent polymorphed first
	if (m_creature->IsPolymorphed())
		return false;
