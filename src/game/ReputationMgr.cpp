        ...
        for (uint32 i = 0; i < MAX_SPILLOVER_FACTIONS; ++i)
        {
            if (repTemplate->faction[i])
            {
                if (m_player->GetReputationRank(repTemplate->faction[i]) <= ReputationRank(repTemplate->faction_rank[i]))
                {
					...
					FactionEntry const *spilloverEntry = sFactionStore.LookupEntry(repTemplate->faction[i]);
                    SetOneFactionReputation(spilloverEntry, spilloverRep, incremental);

					FactionStateList::iterator spilloverFaction = m_factions.find(spilloverEntry->reputationListID);

					if (spilloverFaction != m_factions.end())
					{
						SendState(&spilloverFaction->second);
					}
                }
            }
        }
