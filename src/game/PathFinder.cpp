    ...
    m_sourceUnit(owner), m_navMesh(NULL), m_navMeshQuery(NULL)
{
    ...
    DEBUG_FILTER_LOG(LOG_FILTER_PATHFINDING, "++ PathInfo::PathInfo for %lu \n", m_sourceUnit->GetGUID());
            ...
            !m_sourceUnit->hasUnitState(UNIT_STAT_IGNORE_PATHFINDING))
    {
		...
		// If no valid path is found we try again and move the start point one yard in a random direction.
		if (m_type == PATHFIND_NOPATH || m_type == PATHFIND_INCOMPLETE)
		{
			uint32 rnd = urand(0, 100);

			startPoint.x = startPoint.x + cosf((float)((2 * 3.141592654f / 100) * rnd));
			startPoint.y = startPoint.y + sinf((float)((2 * 3.141592654f / 100) * rnd));
                      ...
                      bool useStraightPath, bool forceDest)
{
    ...
    DEBUG_FILTER_LOG(LOG_FILTER_PATHFINDING, "++ PathInfo::Update() for %lu \n", m_sourceUnit->GetGUID());
    ...
    else
    {
		...
		if (m_type == PATHFIND_NOPATH || m_type == PATHFIND_INCOMPLETE)
		{
			...
			newStart.x = newStart.x + cosf((float)((2 * 3.141592654f / 100) * rnd));
			newStart.y = newStart.y + sinf((float)((2 * 3.141592654f / 100) * rnd));
...
void PathInfo::BuildPolyPath(PathNode startPos, PathNode endPos)
{
    ...
    if (startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
    {
		...
		m_type = (m_sourceUnit->GetTypeId() == TYPEID_UNIT && (((Creature*)m_sourceUnit)->CanFly() | 
			((Creature*)m_sourceUnit)->CanSwim())) ? PathType(PATHFIND_NORMAL | PATHFIND_NOT_USING_PATH) : PATHFIND_NOPATH;
        return;
    }
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (DT_SUCCESS != m_navMeshQuery->closestPointOnPoly(suffixStartPoly, endPoint, suffixEndPoint))
        {
            ...
            sLog.outError("%lu's Path Build failed: invalid polyRef in path", m_sourceUnit->GetGUID());
        ...
        if (!suffixPolyLength || dtResult != DT_SUCCESS)
        {
            ...
            sLog.outError("%lu's Path Build failed: 0 length path", m_sourceUnit->GetGUID());
        }
...
void PathInfo::BuildPointPath(float *startPoint, float *endPoint)
{
    ...
    for (uint32 i = 0; i < pointCount; ++i)
	{
		if (i > 0)
		{
			...
			std::vector<PathingBarrier> const& barriers = sObjectMgr.GetPathingBarrier(m_sourceUnit->GetMapId());
			for (PathingBarrier current_barrier : barriers)
			{
				...
				{
					GameObject* connected_door = m_sourceUnit->GetMap()->GetGameObject(ObjectGuid(HIGHGUID_GAMEOBJECT, connected_door_data->id, current_barrier.guid));

					if (connected_door && connected_door->GetGoState() == GO_STATE_ACTIVE)
						continue;
				}

				double x_in[5] = { current_barrier.x0, current_barrier.x1, current_barrier.x2, pathPoints[(i - 1)*VERTEX_SIZE+2], pathPoints[i*VERTEX_SIZE+2] };
				double y_in[5] = { current_barrier.y0, current_barrier.y1, current_barrier.y2, pathPoints[(i - 1)*VERTEX_SIZE], pathPoints[i*VERTEX_SIZE] };
				double z_in[5] = { current_barrier.z0, current_barrier.z1, current_barrier.z2, pathPoints[(i - 1)*VERTEX_SIZE+1], pathPoints[i*VERTEX_SIZE+1] };

				if (IsLineIntersecting2DPoly(x_in, y_in, z_in)) // Cut the path short if we hit a blocking polygon.
				{
					isIntersecting = true;
					break;
				}
			...
			if (isIntersecting)
			{
				/*float endX = pathPoints[i*VERTEX_SIZE+2], endY = pathPoints[i*VERTEX_SIZE], endZ = pathPoints[i*VERTEX_SIZE+1];
				float startX = pathPoints[(i - 1)*VERTEX_SIZE+2], startY = pathPoints[(i - 1)*VERTEX_SIZE], startZ = pathPoints[(i - 1)*VERTEX_SIZE+1];
				...
				m_pathPoints.set(i, PathNode(endX, endY, endZ));*/

				// Just return without finding a path for the creature and make it evade.
				
				
				if (m_sourceUnit->GetTypeId() != TYPEID_PLAYER)
				{
					Creature* source_creature = m_sourceUnit->GetMap()->GetCreature(m_sourceUnit->GetGUID());
					if (source_creature)
					{
						source_creature->ForcedDespawn(0);
						source_creature->Respawn();
					}
				}
...
bool IsLineIntersecting2DPoly(double x[5], double y[5], double z[5], float *x_int, float *y_int, float *z_int)
{
	
	// Get the intersection point from the solution to the parametric equation for the polygon and the line. The solution was acquired by the following Maple input: solve({t*(x[1]-x[0])+s*(x[2]-x[0])+x[0]-p*(x[4]-x[3])-x[3], t*(y[1]-y[0])+s*(y[2]-y[0])+y[0]-p*(y[4]-y[3])-y[3], t*(z[1]-z[0])+s*(z[2]-z[0])+z[0]-p*(z[4]-z[3])-z[3]}, {p, s, t})
	double p_denominator = (x[3] * y[0] * 

			  z[1] - x[3] * y[1] * z[0] + x[3] * y[1] * z[2] - x[3] * y[2] * z[1]

			   - x[3] * y[0] * z[2] + x[3] * y[2] * z[0] + x[0] * y[1] * z[3]

			   - x[0] * y[3] * z[1] - x[1] * y[0] * z[3] + x[1] * y[2] * z[3]

			   + x[1] * y[3] * z[0] - x[1] * y[3] * z[2] - x[2] * y[1] * z[3]

			   + x[2] * y[3] * z[1] - x[0] * y[2] * z[3] + x[0] * y[3] * z[2]

			   + x[2] * y[0] * z[3] - x[2] * y[3] * z[0] - x[0] * y[1] * z[4]

			   + x[0] * y[4] * z[1] + x[1] * y[0] * z[4] - x[1] * y[2] * z[4]

			   - x[1] * y[4] * z[0] + x[1] * y[4] * z[2] + x[2] * y[1] * z[4]

			   - x[2] * y[4] * z[1] - x[4] * y[0] * z[1] + x[4] * y[1] * z[0]

			   - x[4] * y[1] * z[2] + x[4] * y[2] * z[1] + x[0] * y[2] * z[4]

			   - x[0] * y[4] * z[2] - x[2] * y[0] * z[4] + x[2] * y[4] * z[0]

			   + x[4] * y[0] * z[2] - x[4] * y[2] * z[0]);

	if (p_denominator == 0) // If the denominator is zero there is no solution and no intersection.
		return false;

	double p = - (-x[3] * y[0] * z[1] + x[3] * y[1] * z[0] - x[3] * y[1] * z[2]

			   + x[3] * y[2] * z[1] + x[3] * y[0] * z[2] - x[3] * y[2] * z[0]

			   - x[0] * y[1] * z[3] + x[0] * y[3] * z[1] + x[1] * y[0] * z[3]

			   - x[1] * y[2] * z[3] - x[1] * y[3] * z[0] + x[1] * y[3] * z[2]

			   + x[2] * y[1] * z[3] - x[2] * y[3] * z[1] + x[0] * y[2] * z[3]

			   - x[0] * y[3] * z[2] - x[2] * y[0] * z[3] + x[2] * y[3] * z[0]

			   + y[0] * z[1] * x[2] - y[1] * z[0] * x[2] - y[0] * z[2] * x[1]

			   + y[2] * z[0] * x[1] + x[0] * y[1] * z[2] - x[0] * y[2] * z[1]) / p_denominator;

    double s_denominator = (x[3] * y[0] * z[1]

			   - x[3] * y[1] * z[0] + x[3] * y[1] * z[2] - x[3] * y[2] * z[1]
    ...
    double s = (x[0] * y[1] * z[3]

			   - x[0] * y[1] * z[4] - x[0] * y[3] * z[1] + x[0] * y[3] * z[4]

			   + x[0] * y[4] * z[1] - x[0] * y[4] * z[3] - x[1] * y[0] * z[3]

			   + x[1] * y[0] * z[4] + x[1] * y[3] * z[0] - x[1] * y[3] * z[4]

			   - x[1] * y[4] * z[0] + x[1] * y[4] * z[3] + x[3] * y[0] * z[1]

			   - x[3] * y[0] * z[4] - x[3] * y[1] * z[0] + x[3] * y[1] * z[4]

			   + x[3] * y[4] * z[0] - x[3] * y[4] * z[1] - x[4] * y[0] * z[1]

			   + x[4] * y[0] * z[3] + x[4] * y[1] * z[0] - x[4] * y[1] * z[3]

			   - x[4] * y[3] * z[0] + x[4] * y[3] * z[1]) / s_denominator;

	double t_denominator = (x[3] * y[0] * z[1]
	...
	double t = - (x[0] * y[4] * z[2]

				- x[0] * y[4] * z[3] - x[2] * y[0] * z[3] + x[2] * y[0] * z[4]

				+ x[2] * y[3] * z[0] - x[2] * y[3] * z[4] - x[2] * y[4] * z[0]

				+ x[2] * y[4] * z[3] + x[3] * y[0] * z[2] - x[3] * y[0] * z[4]

				- x[3] * y[2] * z[0] + x[3] * y[2] * z[4] + x[3] * y[4] * z[0]

				- x[3] * y[4] * z[2] - x[4] * y[0] * z[2] + x[4] * y[0] * z[3]

				+ x[4] * y[2] * z[0] - x[4] * y[2] * z[3] - x[4] * y[3] * z[0]

				+ x[4] * y[3] * z[2] - x[0] * y[2] * z[4] - x[0] * y[3] * z[2]

				+ x[0] * y[3] * z[4] + x[0] * y[2] * z[3]) / t_denominator;


	// Check if the found intersection is within the polygon.
	if ( t <= 1 && t >= 0 && s <= 1 && s >= 0 && p <= 1 && p >= 0)
	{
		if (x_int)
			*x_int = (float)(p * (x[4] - x[3]) + x[3]);
		if (y_int)
			*y_int = (float)(p * (y[4] - y[3]) + y[3]);
		if (z_int)
			*z_int = (float)(p * (z[4] - z[3]) + z[3]);
...
bool IsPathingBarrierIntersectedInMap(const WorldObject *caller, float x1, float y1, float z1, float x2, float y2, float z2)
{
	// Check if the supplied line will intersect a barrier.
	bool isIntersecting = false;
	std::vector<PathingBarrier> const& barriers = sObjectMgr.GetPathingBarrier(caller->GetMap()->GetId());
	for (PathingBarrier current_barrier : barriers)
	{
		const GameObjectData* connected_door_data = sObjectMgr.GetGOData(current_barrier.guid);
		if (connected_door_data)
		{
			GameObject* connected_door = caller->GetMap()->GetGameObject(ObjectGuid(HIGHGUID_GAMEOBJECT, connected_door_data->id, current_barrier.guid));

			if (connected_door && connected_door->GetGoState() == GO_STATE_ACTIVE)
				continue;
		}

		double x_in[5] = { current_barrier.x0, current_barrier.x1, current_barrier.x2, x1, x2 };
		double y_in[5] = { current_barrier.y0, current_barrier.y1, current_barrier.y2, y1, y2 };
		double z_in[5] = { current_barrier.z0, current_barrier.z1, current_barrier.z2, z1, z2 };

		if (IsLineIntersecting2DPoly(x_in, y_in, z_in)) // Check if a straight line between the points intersect.
		{
			isIntersecting = true;
			break;
		}
