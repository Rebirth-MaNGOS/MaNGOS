...
{
    if (!entry || entry->AttributesEx2 & SPELL_ATTR_EX2_CANT_CRIT)
        return false;
...
bool IsBinaryEffect(SpellEntry const *spellInfo, SpellEffectIndex effIndex);
...
{
    if (!(spellInfo->Attributes & (SPELL_ATTR_PASSIVE | SPELL_ATTR_HIDDEN)) || !form)
        return false;
...
DiminishingGroup GetDiminishingReturnsGroupForSpell(SpellEntry const* spellproto, bool triggered, uint32 casting_item_entry = 0);
