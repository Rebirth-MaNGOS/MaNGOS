...
{
    public:
        SpellAuraHolder (SpellEntry const* spellproto, bool isTriggered, Unit *target, WorldObject *caster, Item *castItem);
		...
		void SetAuraLostAtShapeShift(bool is_removed) { m_isRemovedOnShapeLost = is_removed; }
...
class MANGOS_DLL_SPEC Aura
{
    ...
    public:
        ...
        bool IsMorePowerfulResistanceApplied(int8 currentSchoolmask);
        Aura* GetMostPowerfulResistance(int8 currentSchoolmask);
		...
		SpellModifier const* GetSpellModifier() { return m_spellmod; }
    ...
    protected:
		...
		int m_pulseResistFrequency;							//How often to pulse resist
		int m_pulseResistTimer;								//How long until the next pulse resist roll
		int m_isPvp;										//Is this a debuff placed on one player by another?
		int m_pulseResistCount;								//In PVP we use this to keep track of how many pulse resists we've passed- 
															//each one adds to the resist bias.
...
class MANGOS_DLL_SPEC CumulativeEffectAura : public SingleEnemyTargetAura
{
	friend Aura* CrateAura(SpellEntry const* spellproto, SpellEffectIndex eff, int32 *currentBasePoints, SpellAuraHolder *holder, Unit *target, Unit *caster, Item* castItem);

    public:
		~CumulativeEffectAura();
		CumulativeEffectAura(SpellEntry const* spellproto, SpellEffectIndex eff, int32 *currentBasePoints, SpellAuraHolder *holder, Unit *target, Unit *caster  = NULL, Item* castItem = NULL);
		void AddCumulativeEffect(float value);
		float GetCumulativeEffect();
		void SetDamageModifier(float value);
		float GetDamageModifier();
    private:
		float m_cumulativeEffect;
...
SpellAuraHolder* CreateSpellAuraHolder(SpellEntry const* spellproto, bool isTriggered, Unit *target, WorldObject *caster, Item *castItem = NULL);
#endif
