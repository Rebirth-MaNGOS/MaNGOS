...
void ObjectMgr::LoadPathingBarriers()
{
	 unsigned long counter = 0;

	 QueryResult *result = WorldDatabase.Query("SELECT guid, map, x0, y0, z0, x1, y1, z1, x2, y2, z2 FROM pathing_barriers");
	 ...
	 if (!result)
	 {
		...
		sLog.outString();
		sLog.outErrorDb(">> Loaded 0 pathing barriers. DB table `pathing_barriers` is empty.");
		return;
	 }
	 ...
	 do
	 {
		 ...
		 float x0 = fields[2].GetFloat();
		 float y0 = fields[3].GetFloat();
	     float z0 = fields[4].GetFloat();
		 float x1 = fields[5].GetFloat();
		 float y1 = fields[6].GetFloat();
		 float z1 = fields[7].GetFloat();
		 float x2 = fields[8].GetFloat();
		 float y2 = fields[9].GetFloat();
		 float z2 = fields[10].GetFloat();

		 PathingBarrier barrier(guid, x0, y0, z0, x1, y1, z1, x2, y2, z2);
		 if (mPathingBarriers.find(map) == mPathingBarriers.end())
		 {
			 std::vector<PathingBarrier> barrierList;
			 barrierList.push_back(barrier);
			 mPathingBarriers[map] = barrierList;
		 }
		 else
			 mPathingBarriers[map].push_back(barrier);
...
void ObjectMgr::LoadDebuffPriorities()
{
	unsigned long counter = 0;

	 QueryResult *result = WorldDatabase.Query("SELECT spellId, overwritten_spellId FROM debuff_priorities");
	 ...
	 if (!result)
	 {
		...
		sLog.outString();
		sLog.outErrorDb(">> Loaded 0 debuff priorities. DB table `debuff_priorities` is empty.");
		return;
	 }
	 ...
	 do
	 {
		 ...
		 uint32 overwritten_spellId = fields[1].GetUInt32();

		 if (mDebuffPriorities.find(spellId) == mDebuffPriorities.end())
		 {
			 std::vector<uint32> overwrites_list;
			 overwrites_list.push_back(overwritten_spellId);
			 mDebuffPriorities[spellId] = overwrites_list;
		 }
		 else
...
void ObjectMgr::FlushRankPoints(uint32 dateTop)
{
    ...
    if (result)
    {
        ...
        while (WeekBegin <= dateTop)
        {
			...
			if (flush && sWorld.getConfig(CONFIG_BOOL_ENABLE_NON_PVP_DECAY))
			{
				DecayNonPvPPlayers(ALLIANCE, false);
				DecayNonPvPPlayers(HORDE, false);
			}
	...
	else if (sWorld.getConfig(CONFIG_BOOL_ENABLE_NON_PVP_DECAY))
	{
		 // If there are no honour standings we loop through all the weeks since the last maintenance and decay everyone.
		 QueryResult *result = CharacterDatabase.Query("SELECT NextMaintenanceDate FROM saved_variables");
		 if (result)
		 {
			 Field* fields = result->Fetch();

			 uint32 last_maintenance_date = fields[0].GetUInt32();

			 while (last_maintenance_date <= dateTop)
			 {
				 DecayNonPvPPlayers(ALLIANCE, true);
				 DecayNonPvPPlayers(HORDE, true);
...
void ObjectMgr::DecayNonPvPPlayers(Team team, bool no_standings)
{
	std::list<std::pair<uint32, float> > updated_rank_points;
	...
	if (team == ALLIANCE)
	{
		players = CharacterDatabase.PQuery("SELECT guid, stored_honor_rating FROM characters WHERE stored_honor_rating > 0 AND race IN (1, 3, 4, 7)");
	}
	else
	{
		players = CharacterDatabase.PQuery("SELECT guid, stored_honor_rating FROM characters WHERE stored_honor_rating > 0 AND race IN (2, 5, 6, 8)");
	}
	...
	if (players)
	{
		...
		do
		{
			fields = players->Fetch();

			uint32 guid = fields[0].GetUInt32();
			float stored_honor_rating = fields[1].GetFloat();

			// Players not in the standing list should have their rank points decay as follows: rank > 7 -> - 2 500 per week. rank < 7 - 10 % per week.
			if (no_standings || std::find_if(list.begin(), list.end(), [&](HonorStanding standing) { return standing.guid == guid; }) == list.end())
			{
				// Above rank 7.
				if (stored_honor_rating > 30000)
				{
					updated_rank_points.push_back(std::make_pair(guid, stored_honor_rating - 2500));
				}
				else // Below rank 7.
				{
					updated_rank_points.push_back(std::make_pair(guid, floor(stored_honor_rating * 0.9f)));
				}
			}

		} while (players->NextRow());
	}

	CharacterDatabase.BeginTransaction();
	for (std::pair<uint32, float> updated_value : updated_rank_points)
	{
		CharacterDatabase.PExecute("UPDATE characters SET stored_honor_rating = '%f' WHERE guid = '%u'", updated_value.second, updated_value.first);
	}
	CharacterDatabase.CommitTransaction();
...
void ObjectMgr::DistributeRankPoints(uint32 team, uint32 dateBegin , bool flush /*false*/)
{
    ...
    for (HonorStandingList::iterator itr = list.begin();itr != list.end() ; ++itr)
    {
        ...
        if (flush)
        {
			float storedHonor = finiteAlways(RP);
			uint32 honorKills = HK+itr->honorKills;

			Player *player = sObjectMgr.GetPlayer(ObjectGuid(HIGHGUID_PLAYER,itr->guid));
			if (player != NULL)
			{
				player->SetStoredHonor(storedHonor);
				player->SetHonorStoredKills(honorKills,true);
			}
            ...
            CharacterDatabase.PExecute("UPDATE characters SET stored_honor_rating = %f , stored_honorable_kills = %u WHERE guid = %u",storedHonor,honorKills,itr->guid);
            CharacterDatabase.CommitTransaction();
        }
    }
