...
Position Transport::GetPrecisePosition()
{
	Position p;
	if (m_next->second.mapid != m_curr->second.mapid || m_next->second.teleport)
	{
		p.x = m_curr->second.x;
		p.y = m_curr->second.y;
		p.z = m_curr->second.z;
	...
	{
		float segmentTime = (m_next->first - m_curr->first) % m_pathTime;
		float percentComplete = ((m_timer - m_curr->first) % m_pathTime) / segmentTime;

		float diffX = m_next->second.x - m_curr->second.x;
		float diffY = m_next->second.y - m_curr->second.y;
		float diffZ = m_next->second.z - m_curr->second.z;

		p.x = m_curr->second.x + (diffX * percentComplete);
		p.y = m_curr->second.y + (diffY * percentComplete);
		p.z = m_curr->second.z + (diffZ * percentComplete);

		G3D::Vector3 originVector(-1,0,0);
		G3D::Vector3 facingVector(diffX,diffY,0);
		facingVector = facingVector.unit();

		float angle = G3D::aCos(originVector.dot(facingVector));
		if (diffY > 0)
			angle *= -1;
...
void Transport::Update( uint32 update_diff, uint32 /*p_time*/)
{
	...
	for(PlayerSet::const_iterator itr = m_passengers.begin(); itr != m_passengers.end();++itr)
	{
		...
		if (p->GetNudgeState() == NUDGESTATE_ACTIVELYNUDGING)
		{
			if (p->m_movementInfo.nextNudgeTime < update_diff)
			{
				p->SendTransportNudge();
			} else
			{
				p->m_movementInfo.nextNudgeTime -= update_diff;
			}
		}
	}
    ...
    while (((m_timer - m_curr->first) % m_pathTime) > ((m_next->first - m_curr->first) % m_pathTime))
    {
		...
		//Fun fact, there's a teleport in the Grom'Gol/Undercity circuit with absolutely no data-side indication!  
		//Yay!  So let's hack it to always teleport before the first node of taxipath 301
		bool doAlwaysTeleport = m_curr->second.teleport;

		if (this->m_goInfo->moTransport.taxiPathId == 301 && m_curr == m_WayPoints.begin())
			doAlwaysTeleport = true;

        // first check help in case client-server transport coordinates de-synchronization
        if (m_curr->second.mapid != GetMapId() || doAlwaysTeleport)
        {
