...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADINSTANCEQUOTA,   "SELECT map_id, time FROM character_instance_quota WHERE guid = '%u'", m_guid.GetCounter());
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_QUESTEXPRATE,        "SELECT rate FROM character_questexprate WHERE guid = '%u'", m_guid.GetCounter());
    return res;
}
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder)
{
	...
	//Custom login delay
	pCurrChar->m_loginDelayTimer = 850;
