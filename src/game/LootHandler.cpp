...
void WorldSession::DoLootRelease(ObjectGuid lguid)
{
    ...
    switch(lguid.GetHigh())
    {
        ...
        case HIGHGUID_UNIT:
        {
			...
			if (loot->GroupLooterGuid == player->GetObjectGuid())
			{
				loot->GroupLooterGuid.Clear();
				loot->GroupLooterGuid = ObjectGuid();
			}

            if (loot->isLooted())
            {
                // for example skinning after normal loot
                pCreature->PrepareBodyLootState(pCreature->IsRewardAllowedForPlayer());
