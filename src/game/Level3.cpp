...
bool ChatHandler::HandleListItemCommand(char* args)
{
    ...
    char *playerName = ExtractArg(&args);
    if (playerName && atoi(playerName) == 0)
        isNameSupplied = true;
    else
    {
        args = playerName;
        isNameSupplied = false;
        playerName = NULL;
    }
    ...
    uint32 count;
    if (!ExtractOptUInt32(&args, count, 10) && !isNameSupplied)
        return false;
    ...
    if (playerName)
    {
        ...
        //          0        1             2             3        4                  5
            ...
            "WHERE ci.item_template='%u' AND ci.guid = characters.guid AND characters.name = '%s' LIMIT %u ",
            item_id,playerName, uint32(count));
    ...
    if(count > 0)
    {
        if (playerName)
        {
            ...
            //          0                     1            2              3               4            5               6
                ...
                "WHERE mail_items.item_template='%u' AND char_s.guid = mail.sender AND char_r.guid = mail.receiver AND mail.id=mail_items.mail_id AND (char_s.name = '%s' OR char_r.name = '%s') LIMIT %u",
                item_id, playerName, playerName, uint32(count));
        }
        else
        {
            result=CharacterDatabase.PQuery(
    ...
    if(count > 0)
    {
        if (playerName)
        {
            ...
            //           0                      1                       2                   3
                ...
                "FROM auction,characters WHERE auction.item_template='%u' AND characters.guid = auction.itemowner AND characters.name = '%s' LIMIT %u",
                item_id, playerName, uint32(count));
        }
        else
        {
...
bool ChatHandler::HandleDamageCommand(char* args)
{
    ...
    if (!*args)
    {
		...
		PartialResistInfo partialResist = m_session->GetPlayer()->MagicSpellPartialResistResult(target,NULL,schoolmask);
        target->CalculateDamageAbsorbAndResist(m_session->GetPlayer(),schoolmask, SPELL_DIRECT_DAMAGE, damage, &absorb, &resist,false,partialResist);

        if (actualDamage <= absorb + resist)
            return true;

        actualDamage -= absorb + resist;

        m_session->GetPlayer()->DealDamageMods(target,actualDamage,&absorb);
        m_session->GetPlayer()->DealDamage(target, actualDamage, NULL, DIRECT_DAMAGE, schoolmask, NULL, false);
...
bool ChatHandler::HandleAuraCommand(char* args)
{
    ...
    SpellAuraHolder *holder = CreateSpellAuraHolder(spellInfo, false, target, m_session->GetPlayer());
...
bool ChatHandler::HandleAddBGCreatureCommand(char* args)
{
	int32 entry, event1, event2;
	float radius;

	if (!ExtractInt32(&args, entry) || entry <= 0)
		return false;

	if (!ExtractInt32(&args, event1))
		return false;

	if (!ExtractInt32(&args, event2))
		return false;

	if (!ExtractFloat(&args, radius) || radius <= 0)
		radius = 10.f;
	...
	QueryResult *result = WorldDatabase.PQuery("SELECT guid FROM creature WHERE map = '%u' AND (POW(position_x - '%f', 2) + POW(position_y - '%f', 2)) <= '%f' AND id = '%u'", 
						    player->GetMap()->GetId(), player->GetPositionX(), player->GetPositionY(), radius * radius, entry);
	if (result)
	{
		uint32 counter = 0;
		...
		do
		{
			...
			QueryResult *exist_result = WorldDatabase.PQuery("SELECT guid FROM creature_battleground WHERE guid = '%u'", guid);
			
			if (!exist_result)
			{
				if (WorldDatabase.PExecute("INSERT INTO creature_battleground (guid, event1, event2) VALUES('%u', '%u', '%u')", guid, event1, event2))
					++counter;
			}

		} while(result->NextRow());

		PSendSysMessage(LANG_CREATURES_INSERTED_IN_BATTLEGROUND, counter, entry);
		
		return true;
	}
	else
	{
		PSendSysMessage(LANG_CREATURES_NOT_FOUND_IN_BATTLEGROUND, entry);
		SetSentErrorMessage(true);
		return false;
	}
}
...
bool ChatHandler::ShowPlayerListHelper(QueryResult* result, uint32* limit, bool title, bool error)
{
    ...
    if (result)
    {
        ...
        do
        {
			...
			uint8 online	 = fields[5].GetUInt8();
            ...
            if (!m_session)
                PSendSysMessage(LANG_CHARACTERS_LIST_LINE_CONSOLE, guid, name.c_str(), race_name, class_name, level, online ? "----Online" : "");
            else
                PSendSysMessage(LANG_CHARACTERS_LIST_LINE_CHAT, guid, name.c_str(), name.c_str(), race_name, class_name, level, online ? "----Online" : "");
...
bool ChatHandler::HandleAccountCharactersCommand(char* args)
{
    ...
    QueryResult *result = CharacterDatabase.PQuery( "SELECT guid, name, race, class, level, online FROM characters WHERE account = %u", account_id);
...
bool ChatHandler::HandleMmapTestArea(char* args)
{
    ...
    {
        PSendSysMessage("Found %li Creatures.", creatureList.size());
