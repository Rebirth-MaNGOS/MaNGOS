...
// Start of edit to address issue occuring when both flags are on the ground.


    //Alliance flag on ground(not in base) (returned to base.)
    if (GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_GROUND && Source->GetTeam() == ALLIANCE && target_obj->GetObjectGuid() == GetDroppedFlagGuid(ALLIANCE) && Source->IsWithinDistInMap(target_obj, 10))
    {
    ...
    //Alliance flag on ground(not in base) (picked up again from ground!)
    if (GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_GROUND && Source->GetTeam() == HORDE && target_obj->GetObjectGuid() == GetDroppedFlagGuid(ALLIANCE) && Source->IsWithinDistInMap(target_obj, 10))
    {
    ...
    //Horde flag on ground(not in base) (returned to base.)
    if (GetFlagState(HORDE) == BG_WS_FLAG_STATE_ON_GROUND && Source->GetTeam() == HORDE && target_obj->GetObjectGuid() == GetDroppedFlagGuid(HORDE) && Source->IsWithinDistInMap(target_obj, 10))
    {
    ...
    //Horde flag on ground(not in base) (picked up again from ground!)
    if (GetFlagState(HORDE) == BG_WS_FLAG_STATE_ON_GROUND && Source->GetTeam() == ALLIANCE && target_obj->GetObjectGuid() == GetDroppedFlagGuid(HORDE) && Source->IsWithinDistInMap(target_obj, 10))
    {
        message_id = LANG_BG_WS_PICKEDUP_HF;
        type = CHAT_MSG_BG_SYSTEM_ALLIANCE;
        PlaySoundToAll(BG_WS_SOUND_HORDE_FLAG_PICKED_UP);
...
void BattleGroundWS::Reset()
{
    ...
    m_ReputationCapture = (isBGWeekend) ? 45 : 35;		// 25 : 15 before
