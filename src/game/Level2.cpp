...
// Add a new waypoint to a creature, the waypoint gets the next available ID.
bool ChatHandler::HandleAddSDZCreatureWaypoint(char* args)
{
	uint32 entry, waittime, pointid;
	float x, y, z;
	Player* player = m_session->GetPlayer();
	...
	QueryResult *result = sScriptMgr.GetScriptdevDB()->PQuery("SELECT MAX(pointid) FROM script_waypoint WHERE entry = '%u' LIMIT 1", entry);
	...
	{
		Field *fields = result->Fetch();
		pointid = fields[0].GetUInt32() + 1; // Get the max pointid and increment it by 1.
	}

	player->GetPosition(x, y, z);

	if (sScriptMgr.GetScriptdevDB()->PExecute("INSERT INTO script_waypoint (entry, pointid, location_x, location_y, location_z, waittime) VALUES('%u', '%u', '%f', '%f', '%f', '%u')", entry, pointid, x, y, z, waittime))
	{
		PSendSysMessage(LANG_CREATURES_ADDED_WAYPOINT, entry, pointid, x, y, z);
		return true;
	}
...
bool ChatHandler::HandleTicketCommand(char* args)
{
	...
	char* second = ExtractLiteralArg(&args);
	...
	// List tickets by number, player name and online status.
	if (strncmp(px, "list", 5) == 0)
	{

		for (int ticket_number = 0; ticket_number <= (int64) sTicketMgr.GetTicketCount() - 1; ticket_number++)
			{
				std::string output;
				std::string time_since_online = "";

				GMTicket* current_ticket = sTicketMgr.GetGMTicketByOrderPos(ticket_number);
			
				std::string name;
				if (!sObjectMgr.GetPlayerNameByGUID(current_ticket->GetPlayerGuid(), name))
					name = GetMangosString(LANG_UNKNOWN);

				Player* current_player = sObjectMgr.GetPlayer(name.c_str());

				if (second && strncmp(second, "online", 7) == 0 && !current_player)
					continue;
				else if (second && strncmp(second, "offline", 8) == 0 && current_player)
					continue;

				char buf[20];
				ACE_OS::itoa(ticket_number + 1, buf, 10);
				output = buf;
				output += ": " + name + " - ";

				if (!current_player)
				{
					QueryResult *result = CharacterDatabase.PQuery("SELECT logout_time FROM characters WHERE guid='%u'", current_ticket->GetPlayerGuid().GetCounter());
					time_t current_time = time(NULL);
					time_t last_online = 0;
					if (result)
					{
						Field* field = result->Fetch();
						last_online = (time_t) field[0].GetUInt64();

						time_t offline_duration = current_time - last_online;

						uint32 days = offline_duration / (60 * 60 * 24);
						uint32 hours = (offline_duration % (60 * 60 * 24)) / (60 * 60);

						char buffer[100];
						sprintf(buffer, " - %ud %uh", days, hours);
						time_since_online = buffer;

					}

				}

				output += current_player ? "Online" : "n.a." + time_since_online;
				SendSysMessage(output.c_str());
			
			}

		if (sTicketMgr.GetTicketCount() == 0)
			SendSysMessage("No tickets to list.");
...
bool ChatHandler::HandleLookupPlayerIpCommand(char* args)
{
    ...
    return LookupPlayerSearchCommand(result, &limit, true);
}

bool ChatHandler::HandleLookupPlayerIpAllCommand(char* args)
{
    char* ipStr = ExtractQuotedOrLiteralArg(&args);
    if (!ipStr)
        return false;
...
bool ChatHandler::LookupPlayerSearchCommand(QueryResult* result, uint32* limit, bool only_online)
{
    if (!result)
    {
	...
	QueryResult *chars;
        if (only_online)
	{
	    chars = CharacterDatabase.PQuery( "SELECT guid, name, race, class, level, online FROM characters WHERE account = %u AND online = '1'", acc_id);
	}
	else
	{
	    chars = CharacterDatabase.PQuery( "SELECT guid, name, race, class, level, online FROM characters WHERE account = %u", acc_id);
	}
...
bool ChatHandler::HandleMmapPathCommand(char* args)
{
    ...
    PSendSysMessage("length %li type %u", pointPath.size(), path.getPathType());
...
bool ChatHandler::HandleMmapLocCommand(char* /*args*/)
{
    ...
    PSendSysMessage("%03u%02i%02i.mmtile", player->GetMapId(), gx, gy);
...
bool ChatHandler::HandleMmapAddPointCommand(char* /*args*/)
{
	Player* player = m_session->GetPlayer();

	if (player)
	{
		player->m_MmapPoints.push_back(Point3D(player->GetPositionX(), player->GetPositionY(), player->GetPositionZ()));
		PSendSysMessage("A new point has been added! There are now %lu points in the list.", player->m_MmapPoints.size());
...
bool ChatHandler::HandleMmapGeneratePolyFileCommand(char* /*args*/)
{
	Player* player = m_session->GetPlayer();
	...
	char buff[200];
	sprintf(buff, "%s_out.poly", player->GetName());

	FILE* polyFile = fopen(buff, "w");
	if (!polyFile)
	{
		PSendSysMessage("Could not open file for writing!");
		return false;
	}

	fprintf(polyFile, "# Points\n");
	fprintf(polyFile, "%lu 3 0 0\n", player->m_MmapPoints.size() * 2);

	for(size_t i = 0; i < player->m_MmapPoints.size(); i++)
	{
		fprintf(polyFile, "%lu %f %f %f\n", 2 * i, player->m_MmapPoints[i].x, player->m_MmapPoints[i].y, player->m_MmapPoints[i].z);
		fprintf(polyFile, "%lu %f %f %f\n", 2 * i + 1, player->m_MmapPoints[i].x, player->m_MmapPoints[i].y, player->m_MmapPoints[i].z - 1);
	}

	fprintf(polyFile, "# Faces\n");
	fprintf(polyFile, "%lu 0\n", player->m_MmapPoints.size() + 2);

	for (size_t i = 0; i < player->m_MmapPoints.size() - 1; i++)
	{
			fprintf(polyFile, "1\n");
			fprintf(polyFile, "4 %lu %lu %lu %lu\n", 2 * i, 2 * (i + 1), 2 * (i + 1) + 1, 2 * i + 1);
	}
	fprintf(polyFile, "1\n");
	fprintf(polyFile, "4 %lu %lu %u %lu\n", 2 * player->m_MmapPoints.size() - 2, 0, 1, 2 * player->m_MmapPoints.size() - 1);

	std::stringstream top, bottom;
	top << player->m_MmapPoints.size() << " ";
	bottom << player->m_MmapPoints.size() << " ";
	for(size_t i = 0; i < player->m_MmapPoints.size(); i++)
	{
		top << i * 2 << " ";
		bottom << i * 2 + 1 << " ";
	...
	bottom << "\n\0";
	fprintf(polyFile, "1\n");
	fprintf(polyFile, "%s", top.str().c_str());
	fprintf(polyFile, "1\n");
	fprintf(polyFile, "%s", bottom.str().c_str());

	fprintf(polyFile, "# Regions\n");
	fprintf(polyFile, "0\n");

	fprintf(polyFile, "# Holes\n");
	fprintf(polyFile, "0\n");

	fclose(polyFile);

	player->m_MmapPoints.clear();

	PSendSysMessage("Points written to file successfully!");
