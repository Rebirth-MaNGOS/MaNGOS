...
public:
	typedef std::map<ObjectGuid, uint32> DamageTrackingMap;
	...
	void SetEvadeMode(bool apply);
	bool IsInEvadeMode() const;

	bool AIM_Initialize();
	bool AIM_InitializeScriptdevAI(); // Force the Creature to get a ScriptdevAI regardless of creature type.
	...
	void RemoveCreatureSpellCooldown(uint32 spellid);
	...
	VendorItemData const* GetVendorTemplateItems(Player *pPlayer) const;
	...
	void PrepareBodyLootState(bool allowed);
	...
	void CallAssistance(uint32 chainAssistCount);
	...
	uint32 m_friendAssistTimeout;
	void SetFriendAssistTimeout(uint32 milliseconds) { m_friendAssistTimeout = milliseconds; }
	uint32 GetFriendAssistTimeout() { return m_friendAssistTimeout; }
	void CallFriendAssistChasingFeared(float fRadius);
	...
	void GetFormationMembers(std::vector<Creature*>& L);
	void InitFormationMovement();

	uint32 ProhibitTimer[MAX_SPELL_SCHOOL];
	void ProhibitSpellSchool(SpellSchoolMask idSchoolMask, uint32 unTimeMs );

	void SetRespawnEnabled(bool respawnEnabled) { m_respawnEnabled = respawnEnabled; }

	void RelocateCreature(float x, float y, float z, float orientation);
...
{
public:
	AssistDelayEvent(ObjectGuid victim, Unit& owner, std::list<Creature*> const& assistants, uint32 chainAssistCount);
...
class InstakillEvent : public BasicEvent
{
public:
	InstakillEvent(Unit &owner) : BasicEvent(), m_owner(owner) {}
	bool Execute(uint64 e_time,uint32 p_time);
private:
	Unit &m_owner;
};
