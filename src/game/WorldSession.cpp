...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
		...
		// Force the removal of Giantstalker's 5/8-bonus, Natures Ally, on pet destruction
		// to avoid double application.
		if (_player->HasAura(21926)) // Remove Nature's Ally's trigger from the player.
		{
			SpellAuraHolder* aura_holder = _player->GetSpellAuraHolder(21926);
			if (aura_holder)
				_player->RemoveSpellAuraHolder(aura_holder, AuraRemoveMode::AURA_REMOVE_BY_DEATH);
		}

		Pet* pet = _player->GetPet();
		if (pet && pet->HasAura(21925))  // Remove Nature's Ally from the pet.
		{
