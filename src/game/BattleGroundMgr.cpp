    ...
    //add players from group to ginfo
    {
        ...
        if (!isPremade && sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_QUEUE_ANNOUNCER_JOIN))
        {
            ...
            if (bg)
            {
                ...
                // System message
                else if (sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_QUEUE_ANNOUNCER_JOIN)==2)
                {
...
bool BattleGroundQueue::InviteGroupToBG(GroupQueueInfo * ginfo, BattleGround * bg, Team side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
        ...
        for(GroupQueueInfoPlayers::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
			...
			// Players in combat should not be invited.
			if (!plr->isInCombat())
			{
				// send status packet
...
void BattleGroundQueue::FillPlayersToBG(BattleGround* bg, BattleGroundBracketId bracket_id)
{
    ...
    */
    ...
    while( abs(diffAli - diffHorde) > 0 && (m_SelectionPools[BG_TEAM_HORDE].GetPlayerCount() > 0 || m_SelectionPools[BG_TEAM_ALLIANCE].GetPlayerCount() > 0) )
    {
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
	...
	// If a player is in combat we just keep prolonging the remind invite.
	if (plr->isInCombat())
	{
		if (WorldTimer::getMSTime() + INVITATION_IN_COMBAT_TIME < m_RemoveTime)
		{
		 BGQueueInviteEvent* inviteEvent = new BGQueueInviteEvent(plr->GetObjectGuid(), m_BgInstanceGUID, m_BgTypeId, m_RemoveTime);
         plr->m_Events.AddEvent(inviteEvent, plr->m_Events.CalculateTime(INVITATION_IN_COMBAT_TIME));		 
		}
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue or in battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN, m_RemoveTime - WorldTimer::getMSTime(), 0);
            plr->GetSession()->SendPacket(&data);
        }
    }
