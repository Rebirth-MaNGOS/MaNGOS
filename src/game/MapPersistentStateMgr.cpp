...
bool MapPersistentState::UnloadIfEmpty()
{
	...
	// If it's a raid the persistent state will already have been unloaded so a check is neccessary.
    if (sMapPersistentStateMgr.GetPersistentState(GetMapId(), GetInstanceId()) && CanBeUnload())
    {
...
void MapPersistentState::ExtendCreatureRespawnTime(uint32 loguid, time_t t)
{
	 RespawnTimes::const_iterator itr = m_creatureRespawnTimes.find(loguid);

	 //Only deal with dead creatures
	 if (itr == m_creatureRespawnTimes.end() || (*itr).second < sWorld.GetGameTime())
		 return;

	 time_t newSpawnTime = (*itr).second+t;
	 SetCreatureRespawnTime(loguid, newSpawnTime);
    ...
    static SqlStatementID updSpawnTime ;

    SqlStatement stmt = CharacterDatabase.CreateStatement(updSpawnTime, "UPDATE creature_respawn SET respawntime=? WHERE guid = ? AND instance = ?");
    stmt.PExecute(uint64(newSpawnTime),loguid, m_instanceid);
...
MapPersistentState* MapPersistentStateManager::AddPersistentState(MapEntry const* mapEntry, uint32 instanceId, time_t resetTime, bool canReset, bool load /*=false*/, bool initPools /*= true*/)
{
    ...
    DEBUG_LOG("MapPersistentStateManager::AddPersistentState: mapid = %d, instanceid = %d, reset time = %lu, canRset = %u", mapEntry->MapID, instanceId, resetTime, canReset ? 1 : 0);
...
void MapPersistentStateManager::_ResetOrWarnAll(uint32 mapid, bool warn, uint32 timeLeft)
{
    ...
    for(MapManager::MapMapType::const_iterator mitr = maps.lower_bound(MapID(mapid)); mitr != iter_last; ++mitr)
    {
		...
		// When raids reset the persistant state is deleted. Any remaining players in the instance will therefore crash the server if not removed.
		if (!warn && map2->IsRaid())
		{
			Map::PlayerList const& player_list = map2->GetPlayers();
			std::vector<Player*> remove_player_list;

			for ( Map::PlayerList::const_iterator itr = player_list.begin(); itr != player_list.end(); ++itr)
				remove_player_list.push_back(itr->getSource());
			
			for (Player* current_player : remove_player_list)
				current_player->TeleportToHomebind();
		}
...
void MapPersistentStateManager::LoadCreatureRespawnTimes()
{
	...
	// Creatures on Kalimdor do not belong to an instance and therefore no mapId is loaded along with the instance data.
	// Catch all Kalimdor creatures and set mapId = 1.
	if (data->mapid == 1 && mapId == 0 && instanceId == 0) 
	{
	    mapId = 1;
	}
	else if (mapId != data->mapid)
            continue;
