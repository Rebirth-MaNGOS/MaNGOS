...
void AuctionHouseObject::BuildListOwnerItems(WorldPacket& data, Player* player, uint32 startIndex, uint32& count, uint32& totalcount)
{
    for (AuctionEntryMap::const_iterator itr = AuctionsMap.begin(); itr != AuctionsMap.end(); ++itr)
    {
        AuctionEntry *Aentry = itr->second;
        ...
        {
            if (totalcount >= startIndex && count < 50)
			{
