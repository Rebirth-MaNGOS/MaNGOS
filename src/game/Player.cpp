...
SpellModifier::SpellModifier( SpellModOp _op, SpellModType _type, int32 _value, SpellEntry const* spellEntry, SpellEffectIndex eff, int16 _charges /*= 0*/ ) : op(_op), type(_type), charges(_charges), value(_value), spellId(spellEntry->Id), laterDeletion(false), lastAffected(NULL)
{
    mask = sSpellMgr.GetSpellAffectMask(spellEntry->Id, eff);
}

SpellModifier::SpellModifier( SpellModOp _op, SpellModType _type, int32 _value, Aura const* aura, int16 _charges /*= 0*/ ) : op(_op), type(_type), charges(_charges), value(_value), spellId(aura->GetId()), laterDeletion(false), lastAffected(NULL)
{
...
Player::Player (WorldSession *session): Unit(), m_mover(this), m_camera(this), m_reputationMgr(this)
{
	...
	m_loginDelayTimer = DISABLED_MIRROR_TIMER;
	...
	if (sWorld.getConfig(CONFIG_UINT32_SENTINEL_ACTION_LEVEL) != 0)
		m_sentinel = new Sentinel(this);
	else
		m_sentinel = nullptr;
...
{
    i_AI = new PetAI(reinterpret_cast<Creature*>(this));
    charmedCooldownTimer = 0;
}

void Player::DeleteAI()
{
    i_AI = NULL;
    delete i_AI;
}

void Player::SetCharmedCDTimer(uint32 mSec)
{
    charmedCooldownTimer = mSec;
}

uint32 Player::GetCharmedCDTimer()
{
    return charmedCooldownTimer;
}
...
uint32 Player::EnvironmentalDamage(EnvironmentalDamageType type, uint32 damage)
{
	...
	{
		PartialResistInfo resistInfo = this->MagicSpellPartialResistResult(this,NULL,SPELL_SCHOOL_MASK_NATURE);
        CalculateDamageAbsorbAndResist(this, SPELL_SCHOOL_MASK_NATURE, DIRECT_DAMAGE, damage, &absorb, &resist,false,resistInfo);
	}
...
void Player::Update( uint32 update_diff, uint32 p_time )
{
    ...
    // Check and update the player's position for the bot detection.
    if (m_sentinel)
		m_sentinel->UpdatePosition(GetPositionX(), GetPositionY(), GetOrientation());
	...
	// Custom login delay
	if (m_loginDelayTimer != DISABLED_MIRROR_TIMER)
	{
		if (m_loginDelayTimer <= (int32) p_time)
			m_loginDelayTimer = DISABLED_MIRROR_TIMER;
		else
			m_loginDelayTimer -= p_time;
	}
...
void Player::SetDeathState(DeathState s)
{
    ...
    if(s == JUST_DIED && cur)
    {
		...
		if (getClass() != CLASS_WARLOCK || (getClass() == CLASS_WARLOCK && !InBattleGround()))
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
	...
	} else if (m_transport && m_movementInfo.transportNudgeState == NUDGESTATE_NONUDGE)
	{
		m_movementInfo.transportNudgeState = NUDGESTATE_AWAITINGACCEPT;
	}
    ...
    if ((GetMapId() == mapid) && (!m_transport || (options & TELE_TO_SPELL)))
    {
	...
	// Update the player's position in Sentinel.
	if (GetSentinel())
		GetSentinel()->SetPlayerPosition(x, y);
...
void Player::SupercedeSpell(uint32 old_spell_id, uint32 new_spell_id)
{
	this->RemoveAurasDueToSpell(old_spell_id);
    // update spell ranks in spellbook and action bar
    WorldPacket data(SMSG_SUPERCEDED_SPELL, (4));
    data << uint16(old_spell_id);
    data << uint16(new_spell_id);
    GetSession()->SendPacket( &data );
}
    ...
    if (itr != m_spells.end())
    {
        ...
        if(itr->second.active != active && itr->second.state != PLAYERSPELL_REMOVED && !itr->second.disabled)
        {
            ...
            else if(IsInWorld())
            {
                ...
                {
					SupercedeSpell(spell_id, next_active_spell_id);
                }
                else
                {
                ...
                if( sSpellMgr.IsRankSpellDueToSpell(spellInfo, itr2->first) )
                {
                    if(itr2->second.active)
                    {
                        if(sSpellMgr.IsHighRankOfSpell(spell_id,itr2->first))
                        {
                            ...
                            {
								SupercedeSpell(itr2->first, spell_id);
                            }
                        ...
                        else if(sSpellMgr.IsHighRankOfSpell(itr2->first,spell_id))
                        {
                            if(IsInWorld())                 // not send spell (re-/over-)learn packets at loading
                            {
                                SupercedeSpell(spell_id, itr2->first);
                            }
...
void Player::removeSpell(uint32 spell_id, bool disabled, bool learn_low_rank)
{
	...
	// Make sure to remove the combustion aura for mages.
	if (spell_id == 11129)
    ...
    if (uint32 prev_id = sSpellMgr.GetPrevSpellInChain (spell_id))
    {
        ...
        else if (cur_active && sSpellMgr.IsRankedSpellNonStackableInSpellBook(spellInfo))
        {
            ...
            if (prev_itr != m_spells.end())
            {
                ...
                {
                    if (learn_low_rank && addSpell(prev_id,true,false,prev_itr->second.dependent,prev_itr->second.disabled))
                    {
                        // downgrade spell ranks in spellbook and action bar
						SupercedeSpell(spell_id, prev_id);
                        prev_activate = true;
                    }
					prev_itr->second.active = true;
                }
            }
        }
    }
...
uint32 Player::resetTalentsCost() const
{
    ...
    if(m_resetTalentsCost < 1*GOLD / 20)
        return 1*GOLD / 20;
    // then 5 gold
    else if(m_resetTalentsCost < 5*GOLD / 20)
        return 5*GOLD / 20;
    // After that it increases in increments of 5 gold
    else if(m_resetTalentsCost < 10*GOLD / 20)
        return 10*GOLD / 20;
    else
    {
        ...
        if(months > 0)
        {
            ...
            int32 new_cost = int32((m_resetTalentsCost) - 5*GOLD*months / 20);
            // to a minimum of 10 gold.
            return uint32(new_cost < 10*GOLD / 20 ? 10*GOLD / 20 : new_cost);
        }
        else
        {
            // After that it increases in increments of 5 gold
            int32 new_cost = m_resetTalentsCost + 5*GOLD / 20;
            // until it hits a cap of 50 gold.
            if(new_cost > 50*GOLD / 20)
                new_cost = 50*GOLD / 20;
            return new_cost;
        }
    }
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
	...
	// Reapply all the effects of equiped items.
    ...
    for(size_t setindex = 0; setindex < ItemSetEff.size(); ++setindex)
    {
        ...
        for(uint32 y=0;y<8; ++y)
        {
			...
			ApplyEquipSpell(spellInfo, NULL, false);
            ApplyEquipSpell(spellInfo,NULL,true); 
        }
    }
...
void Player::KillPlayer()
{
	...
	m_extraAttacks = 0;										// Reset the player's extra attacks on death.
...
void Player::UpdateCombatSkills(Unit *pVictim, WeaponAttackType attType, bool defence)
{
	...
	uint32 greenLevel = MaNGOS::XP::GetLastLevelWhenGreen(moblevel);

	uint32 effectivePlayerLevel = std::min(plevel, greenLevel);
	uint32 skillCapInt = effectivePlayerLevel * 5;
	uint32 skillValueInt = (defence ? GetBaseDefenseSkillValue() : GetBaseWeaponSkillValue(attType));

	if (skillValueInt >= skillCapInt)
		return;

	float skillCap = float(skillCapInt);
	float skillValue = float(skillValueInt);

	float intellect = GetStat(STAT_INTELLECT);
	float intPerLevel = std::sqrt(intellect/((float)std::max(plevel,20U)));

	float freebieCapContributionFromInt = (defence)?0: (skillCap * ((0.45f * (intPerLevel + 0.01f)) / (intPerLevel + 2.2f))); 

	float freebieCap = skillCap - ((skillCap/3.0f) - freebieCapContributionFromInt);

	float chanceOfSkillup = 100.0f;

	if (skillValue > freebieCap)
	{
		float chanceOfSkillupComponent1 = (0.5f + skillValue-freebieCap) * ((0.5f + (skillCap - freebieCap))/100);
		chanceOfSkillup = pow(0.96f,chanceOfSkillupComponent1) * (1.0f - ((0.5f + skillValue - freebieCap)/(skillCap - freebieCap))) * 100.0f;
	}

    if(roll_chance_f(chanceOfSkillup))
    {
        if(defence)
            UpdateDefense();
        else
...
void Player::SendTransportNudge()
{
	m_movementInfo.transportNudgeState = NUDGESTATE_ACTIVELYNUDGING;
	m_movementInfo.nextNudgeTime = 2000;

	Position realTransPos = GetTransport()->GetPrecisePosition();
	float transOffO = float(GetTransOffsetO());

	G3D::Vector3 offset(GetTransOffsetX(),GetTransOffsetY(),GetTransOffsetZ());

	G3D::Vector3 outOffset = offset * (G3D::Matrix3)G3D::Matrix3::fromEulerAnglesXYZ(0,0,-realTransPos.o);

	float playerOrientation = realTransPos.o + transOffO;

	float _2pi = G3D::pi() * 2;
	if (playerOrientation < 0)
		playerOrientation += _2pi;
	else if (playerOrientation > _2pi)
		playerOrientation -= _2pi;

	this->NearTeleportTo(realTransPos.x + outOffset.x,realTransPos.y + outOffset.y,realTransPos.z + outOffset.z+0.02f,playerOrientation,true);
}
...
void Player::UpdateHonor()
{
	...
	// If the player has more than the lowest number of kills required to be included in the standings during its lifetime the lowest possible rank should be rank 1.
	if (prk.positive && (prk.visualRank == 0 || prk.visualRank == 1))
	{
		if ( GetHonorStoredKills(true) > sWorld.getConfig(CONFIG_UINT32_MIN_HONOR_KILLS))
		{
			prk.visualRank = 1;
			prk.rank = 5;
		}
...
void Player::ApplyEquipSpell(SpellEntry const* spellInfo, Item* item, bool apply, bool form_change)
{
    ...
    else
    {
        ...
        else
		{
			...
			SpellAuraHolder* found_aura_holder = NULL;
			for (auto current_aura_item : m_spellAuraHolders)
			{
				if (current_aura_item.first == spellInfo->Id && !current_aura_item.second->GetCastItemGuid())
				{
					found_aura_holder = current_aura_item.second;
					break;
				}
			}
			
			if (found_aura_holder)
				RemoveSpellAuraHolder(found_aura_holder);
		}
    }
}
...
void Player::CastItemCombatSpell(Unit* Target, WeaponAttackType attType)
{
    ...
    for (int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        {
            chance = GetWeaponProcChance(attType);
        }
    ...
    switch(guid.GetHigh())
    {
        ...
        case HIGHGUID_CORPSE:                               // remove insignia
        {
            ...
            if (!bones->lootForBody)
            {
                ...
                Player *corpseOwner = sObjectMgr.GetPlayer(bones->GetOwnerGuid());
                if (corpseOwner != NULL)          
                    GetBattleGround()->GetInsigniaLoot(loot,corpseOwner,this);
...
void Player::SetWarlockPetBeforeDeath()
{
	if (getClass() == CLASS_WARLOCK)
	{
		Pet* current_pet = GetPet();

		if (current_pet)
		{
			switch(current_pet->GetNativeDisplayId())
			{
			case 4449:
				m_warlockPetBeforeDeath = PET_IMP;
				break;
			case 850:
				m_warlockPetBeforeDeath = PET_FELHUNTER;
				break;
			case 4162:
				m_warlockPetBeforeDeath = PET_SUCCUBUS;
				break;
			case 1132:
				m_warlockPetBeforeDeath = PET_VOIDWALKER;
				break;
			default:
				m_warlockPetBeforeDeath = PET_NO_PET;
			}			
...
void Player::SetVirtualItemSlot( uint8 i, Item* /*item*/)
{
	...
	// Do NOT handle enchantment charges here!
    //if(i < 2 && item)
    //{
...
void Player::PrepareGossipMenu(WorldObject *pSource, uint32 menuId)
{
    ...
    for(GossipMenuItemsMap::const_iterator itr = pMenuItemBounds.first; itr != pMenuItemBounds.second; ++itr)
    {
        ...
        if (pSource->GetTypeId() == TYPEID_UNIT)
        {
            ...
            switch(itr->second.option_id)
            {
                ...
                case GOSSIP_OPTION_VENDOR:
                {
                    ...
                    VendorItemData const* tItems = pCreature->GetVendorTemplateItems(this);
        ...
        {
			//Game objects are considered quest givers and gossipers, but nothing else
            GameObject *pGo = (GameObject*)pSource;
			uint32 goFlags = UNIT_NPC_FLAG_GOSSIP | UNIT_NPC_FLAG_QUESTGIVER;
            ...
            switch(itr->second.option_id)
            {
                case GOSSIP_OPTION_QUESTGIVER:
                    ...
                    break;
                case GOSSIP_OPTION_GOSSIP:
					if ( ( goFlags & itr->second.npc_option_npcflag) == itr->second.npc_option_npcflag)
					{
...
bool Player::CanRewardQuest(Quest const *pQuest, uint32 reward, bool msg) const
{
    ...
    if (pQuest->GetRewChoiceItemsCount() > 0)
    {
        if (pQuest->RewChoiceItemId[reward])
        {
            ...
            if (res != EQUIP_ERR_OK && res != EQUIP_ERR_INVENTORY_FULL)
            {
...
void Player::RewardQuest(Quest const *pQuest, uint32 reward, Object* questGiver, bool announce)
{
	...
	//Use custom player specific XP rate, if config rate is different to 1x and custom rate is valid (either 1x or 2x)
	float rate = sWorld.getConfig(CONFIG_FLOAT_RATE_XP_QUEST);
	if (rate != 1.0f)
	{
		uint32 customRate = m_XPrate;
		if (customRate == 1 || customRate == 2)
...
bool Player::SatisfyQuestSkill(Quest const* qInfo, bool msg) const
{
	...
	switch (qInfo->GetQuestId())
	{
		...
		case 5306:
		case 5307:
			if (!HasSpell(9787))		//Requires player to be a weapon smith
				return false;
			break;
		default:
			break;
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
    ...
    _LoadInstanceQuota(holder->GetResult(PLAYER_LOGIN_QUERY_LOADINSTANCEQUOTA));
...
void Player::_LoadAuras(QueryResult *result, uint32 timediff)
{
    ...
    if(result)
    {
        do
        {
            ...
            SpellAuraHolder *holder = CreateSpellAuraHolder(spellproto, false, this, NULL);
...
void Player::_LoadInstanceQuota(QueryResult *result)
{
    if (result)
    {
	m_InstanceResetLists.clear();
	...
	do
	{
	    ...
	    m_InstanceResetLists[map_id].push_back(time);
	} while (result->NextRow());
    }
}
...
bool Player::ValidateInventory()
{
    ...
    if (error)
    {
        ...
        sWorld.SendAntiCheatMessageToGMs(GetName(), "Player's inventory was invalid and the player has been banned! This could be a possible duplication attempt!");
        ChatHandler(this).SendSysMessage(LANG_ITEM_SAVE_FAILED);
        sTicketMgr.Create(GetObjectGuid(), "This player has triggered an inventory fail state and been banned! Most likely an attempt to duplicate items.", true);
        sWorld.BanAccount(BAN_CHARACTER, GetName(), 0, "Triggered an inventory fail state! Most likely an attempt to duplicate items.", "Anti-Cheat");
        return false;
    }
...
DungeonPersistentState* Player::GetBoundInstanceSaveForSelfOrGroup(uint32 mapid)
{
    ...
    if ((!pBind || !pBind->perm) && IsBelowResetQuotaForInstance(mapid))
    {
...
void Player::ConvertInstancesToGroup(Player *player, Group *group, ObjectGuid player_guid)
{
    ...
    if (player)
    {
        for (BoundInstancesMap::iterator itr = player->m_boundInstances.begin(); itr != player->m_boundInstances.end();)
        {
            ...
            if (!itr->second.perm && player->IsBelowResetQuotaForInstance(itr->second.state->GetMapId()))
            {
...
void Player::SaveToDB()
{
	...
	// Perform a check that an account does not try to save another account's character.
	QueryResult *chkChar = CharacterDatabase.PQuery("SELECT account FROM characters WHERE guid = %u", GetGUIDLow());

	if (chkChar)
	{
		Field* charField = chkChar->Fetch();

		uint32 accountID = charField->GetUInt32();

		// If the account saving the character isn't the same account that own the character the saving account is exploiting and is perma banned.
		if (accountID != GetSession()->GetAccountId())
		{
			...
			sprintf(buff, "The account %u tried to save a character belonging to the account %u as its own.", GetSession()->GetAccountId(), accountID);
			//sWorld.BanAccount(BAN_ACCOUNT, GetSession()->GetPlayerName(), 0, buff, "System"); // A kick should suffice. We'll leave the banning to the GMs.

			// I think the Warden log is a good place to gather all anti-cheat messages.
...
void Player::_SaveAuras()
{
    ...
    for(SpellAuraHolderMap::const_iterator itr = auraHolders.begin(); itr != auraHolders.end(); ++itr)
    {
        ...
        if (!holder->IsPassive() && !(holder->GetSpellProto()->Attributes & SPELL_ATTR_HIDDEN && holder->GetCastItemGuid()) && !IsChanneledSpell(holder->GetSpellProto()) && (holder->GetCasterGuid() == GetObjectGuid() || !holder->IsSingleTarget()))
        {
            int32  damage[MAX_EFFECT_INDEX];
            uint32 periodicTime[MAX_EFFECT_INDEX];
...
void Player::_SaveInstanceQuota()
{
    CharacterDatabase.PExecute("DELETE FROM character_instance_quota WHERE guid = '%u'", GetGUIDLow());

    for (std::pair<uint32, std::list<time_t> > queue_pair : m_InstanceResetLists)
    {
        for (time_t current_time : queue_pair.second)
        {
            CharacterDatabase.PExecute("INSERT INTO character_instance_quota (guid, map_id, time) VALUES (%u, %u, %lu)", GetGUIDLow(), queue_pair.first, current_time);
        }
    }
}
...
void Player::ResetInstances(InstanceResetMethod method)
{
    ...
    for (BoundInstancesMap::iterator itr = m_boundInstances.begin(); itr != m_boundInstances.end();)
    {
        ...
        if (!IsBelowResetQuotaForInstance(entry->MapID))
		{
			MapEntry const* mapEntry = sMapStore.LookupEntry(entry->MapID);
			ChatHandler(this).PSendSysMessage("Reset failed! You have reset the %s more than five times the last hour!", mapEntry ? mapEntry->name[GetSession()->GetSessionDbcLocale()] : "<unknown>" );
			++itr;
			continue;
		}
	
		AddToResetQuotaList(entry->MapID);
...
bool Player::IsBelowResetQuotaForInstance(uint32 instanceMapId)
{
    time_t current_time = time(0);

    std::list<time_t> &reset_list = m_InstanceResetLists[instanceMapId];

    auto itr = reset_list.begin();
    while (itr != reset_list.end())
    {
        if (*itr < current_time - 3600) // If a time has expired, i.e. was set more than an hour ago we remove it.
        {
            itr = reset_list.erase(itr);
        }
        else
            break; // Since the times are stored in growing order we can quit here.
    }
    
    // We do not permit more than 5 resets of an instance per hour.
    if (reset_list.size() >= 5)
	return false;
...
void Player::Whisper(const std::string& text, uint32 language, ObjectGuid receiver)
{
    ...
    //PlayerSocial *psocial = new PlayerSocial();
...
void Player::RemoveSpellModsOnSpellSuccess(Spell const* spell)
{
    if(!spell)
        return;
...
void Player::HandleStealthedTrapsDetection()
{
	std::list<GameObject*> traps;

	MaNGOS::AnyStealthedCheck u_check(this);
	MaNGOS::GameObjectListSearcher<MaNGOS::AnyStealthedCheck> searcher(traps,u_check);
	Cell::VisitAllObjects(this,searcher,MAX_PLAYER_STEALTH_DETECT_RANGE);

	WorldObject const* viewPoint = GetCamera().GetBody();

	for (std::list<GameObject*>::const_iterator i = traps.begin(); i != traps.end(); ++i)
	{
        bool hasAtClient = HaveAtClient((*i));
        bool hasDetected = (*i)->isVisibleFor(this, viewPoint);
...
//Used to trigger the cooldown client-side.
void Player::SendCooldownUpdate(){
	 // Send activate cooldown timer (possible 0) at client side
		WorldPacket data(SMSG_COOLDOWN_EVENT, (4+8));
		data << uint32(4074);
		data << GetObjectGuid();
...
bool Player::IsVisibleInGridForPlayer( Player* pl ) const
{
	...
	// Same raid or group should see ghost
...
void Player::UpdateVisibilityOf(WorldObject const* viewPoint, WorldObject* target)
{
    if(HaveAtClient(target))
    {
        if(!target->isVisibleForInState(this, viewPoint, true))
        {
			...
			{
				this->InterruptSpellsTargettingUnit(false,target->GetObjectGuid());
			}
...
void Player::ApplyEquipCooldown( Item * pItem )
{
    ...
    for(int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        // Make sure a longer cooldown isn't overwritten by a shorter one.
        if (m_spellCooldowns[spellData.SpellId].end >= time(NULL) + 30)
            return;
...
uint32 Player::GetMaxLevelForBattleGroundBracketId(BattleGroundBracketId bracket_id, BattleGroundTypeId bgTypeId)
{
	...
	uint32 gameMax = DEFAULT_MAX_LEVEL;

	if (bgTypeId == BATTLEGROUND_AV)
		return gameMax;

	return std::min(gameMax,GetMinLevelForBattleGroundBracketId(bracket_id, bgTypeId) + 9);
}
...
void Player::RemoveMovementFlag(MovementFlags f)
{
    m_movementInfo.SetMovementFlags((MovementFlags)(m_movementInfo.GetMovementFlags() & (f ^ 0xFFFFFFF)));
}
