...
void World::LoadConfigSettings(bool reload)
{
    ...
    setConfig(CONFIG_UINT32_MAX_ASSIST_CHAIN_COUNT, "CreatureAssistMaxChainCount", 3);
    ...
    setConfig(CONFIG_BOOL_ACTIVE_CONTINENT_MOBS, "ActiveContinentMobs", false);
    setConfig(CONFIG_UINT32_ACTIVE_MOB_UPDATE_THREADS, "ActiveMobUpdateThreads", 1);
    ...
    setConfig(CONFIG_UINT32_EVADE_RESET_TIMER, "EvadeResetTimer", 10000);
    ...
    setConfigMinMax(CONFIG_UINT32_BATTLEGROUND_QUEUE_ANNOUNCER_JOIN,   "Battleground.QueueAnnouncer.Join", 0, 0, 3);
	...
	// Sentinel
	setConfig(CONFIG_UINT32_SENTINEL_ACTION_LEVEL, "Sentinel.ActionLevel", 0);
	setConfig(CONFIG_UINT32_SENTINEL_BAN_TIME, "Sentinel.BanLength", 900000);

	setConfig(CONFIG_BOOL_ENABLE_NON_PVP_DECAY, "PvP.EnableNonPvPDecay", true);
}
...
void World::SetInitialWorldSettings()
{
	...
	sLog.outString("Loading debuff priorities...");
	sObjectMgr.LoadDebuffPriorities();
...
void World::SendAntiCheatMessageToGMs(const char* player_name, const char* check_info)
{
	...
	for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
	{
		if (itr->second->GetSession()->GetSecurity() >= SEC_GAMEMASTER && itr->second->isAcceptTickets())
			ChatHandler(itr->second).PSendSysMessage(LANG_ANTI_CHEAT_REPORT, player_name, player_name, check_info);
	}
}
