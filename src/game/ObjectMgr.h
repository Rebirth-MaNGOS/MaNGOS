...
/* For putting up barriers that prevent pathing and blinking. 
The zero index coordinates is the corner of the rectangle/square/quadrilateral with 
the two other set of coordinates building the sides. */
struct PathingBarrier
{
	float x0, y0, z0;
	float x1, y1, z1;
	float x2, y2, z2;
	...
	PathingBarrier(uint32 guid_in, float x0_in, float y0_in, float z0_in, float x1_in, float y1_in, float z1_in, float x2_in, float y2_in, float z2_in) :
		x0(x0_in), y0(y0_in), z0(z0_in), x1(x1_in), y1(y1_in), z1(z1_in), x2(x2_in), y2(y2_in), z2(z2_in), guid(guid_in) {};
};

typedef UNORDERED_MAP<uint32, std::vector<PathingBarrier>> PathingBarriers;

/* For debuff overwrite priorities when the debuff cap of 16 is hit on a mob or player. 
   For each spell we save a list of spells it's supposed to overwrite. */
typedef UNORDERED_MAP<uint32, std::vector<uint32>> DebuffPriorityMap;
...
class ObjectMgr
{
    ...
    public:
		...
		void LoadPathingBarriers();

		void LoadDebuffPriorities();
		...
		void DecayNonPvPPlayers(Team team, bool no_standings);
		...
		std::vector<PathingBarrier> const& GetPathingBarrier(uint32 map)
		{
			return mPathingBarriers[map];
		}

		const std::vector<uint32> *GetDebuffPriorityForSpell(uint32 spellId)
		{
			if (mDebuffPriorities.find(spellId) != mDebuffPriorities.end())
				return &mDebuffPriorities[spellId];
			else
				return NULL;
		}
    ...
    private:
		...
		PathingBarriers mPathingBarriers;
		DebuffPriorityMap mDebuffPriorities;
...
MANGOS_DLL_SPEC const CreatureData* GetCreatureData(uint32 guid);
