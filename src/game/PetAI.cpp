...
void PetAI::UpdateAI(const uint32 diff)
{
    ...
    if (!m_creature->IsNonMeleeSpellCasted(false))
    {
        ...
        if(m_creature->GetTypeId() != TYPEID_PLAYER)
        {
        ...
        {
            // Player spell handling.
            Player* controlledPlayer = reinterpret_cast<Player*>(m_creature);
            PlayerSpellMap playerSpells = controlledPlayer->GetSpellMap();
            Unit* victim = m_creature->getVictim();

			// If the player is set to react aggressively but the owner is dead we pick a random target within 50 yd to attack.
			if (controlledPlayer->GetCharmInfo() && controlledPlayer->GetCharmInfo()->GetReactState() == REACT_AGGRESSIVE && !controlledPlayer->getVictim())
			{
				std::list<Player*> targets;

				MaNGOS::AnyHostileUnitInObjectRangeCheck u_check(controlledPlayer, controlledPlayer, 50.f);
                MaNGOS::PlayerListSearcher<MaNGOS::AnyHostileUnitInObjectRangeCheck> checker(targets, u_check);
                Cell::VisitWorldObjects(controlledPlayer, checker, 50.0f);
				...
				if (!targets.empty())
				{
					...
					m_creature->Attack(*itr, true);
					if (m_creature->GetMotionMaster())
						m_creature->GetMotionMaster()->MoveChase(*itr);
				}

			}

            if(!victim) // Quit if there isn't a valid victim.
                return;

            if(!playerSpells.empty())
            {
                // Get player spells and choose one at random that has a CD less than or equal to 10 min.
                PlayerSpellMap::const_iterator iter = playerSpells.begin();
                uint32 random = urand(0, playerSpells.size() - 1);
                ...
                {
                    DoCastSpellIfCan(victim, spellInfo->Id, false);
                    controlledPlayer->SetCharmedCDTimer(3000);
                    return; // Make sure we do not perform a melee attack if we cast a spell.
                }
                else
                {
                    controlledPlayer->SetCharmedCDTimer(controlledPlayer->GetCharmedCDTimer() - diff);
                }
            ...
            m_creature->SetOrientation(m_creature->GetAngle(victim));           
            if(controlledPlayer->GetCharmedCDTimer() > diff && m_creature->isAttackReady() && m_creature->CanReachWithMeleeAttack(m_creature->getVictim()))
            {
                m_creature->AttackerStateUpdate(m_creature->getVictim());
                m_creature->resetAttackTimer(); // Perform a melee attack.
            }
