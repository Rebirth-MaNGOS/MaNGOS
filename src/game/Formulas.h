...
namespace MaNGOS
{
    namespace Honor
    {
        ...
        inline HonorRankInfo CalculateRankInfo(HonorRankInfo prk)
        {
            ...
            if (prk.rank != 0)
            {
				...
				if (prk.positive && prk.maxRP == 0) // The special case of rank 0/1.
				{
					prk.maxRP = 2000;
					prk.minRP = 0;
				}
        ...
        inline HonorScores GenerateScores(HonorStandingList standingList,uint32 team)
        {
            ...
            float absoluteMin = 1.0f;
            for (int8 group=13; group>=0; group--)
            {
                sc.BRK[group] = std::max((float) floor( (sc.BRK[group] * standingList.size()) + 0.5f), absoluteMin++);
            }
            ...
            for (uint8 i=1;i<=13;i++) {
              ...
              sc.FX[i] = tempSt ? tempSt->honorPoints : 0;

              if (sc.BRK[i] == 0)
                  break;
            }

            tempSt = sObjectMgr.GetHonorStandingByPosition(1, team);

            if (tempSt)
                sc.FX[14] = tempSt->honorPoints;
            else
                sc.FX[14] = 0;
        ...
        inline float HonorableKillPoints( Player *killer, Player *victim, uint32 groupsize)
        {
			...
			// Put a lower limit of 1 000 on the rank points when calculating gained honour for a kill.
			// This to avoid huge numbers in the calculations for people with low rank points.
			uint32 victim_rank_points = victim->GetRankPoints() > 1000 ? victim->GetRankPoints() : 1000;
			uint32 killer_rank_points = killer->GetRankPoints() > 1000 ? killer->GetRankPoints() : 1000;

            float diff_honor = (victim_rank_points / killer_rank_points) + 1;
    ...
    namespace XP
    {
		...
		//Get the last green level for a mob
		inline uint32 GetLastLevelWhenGreen(uint32 mob_level)
		{
			int below40Level = ((10 * mob_level) + 50)/9;

			if (below40Level < 40)
				return below40Level+1;

			return (((5*mob_level) + 5)/4) + 1;
		}
        ...
        inline uint32 Gain(Player *pl, Unit *u)
        {
			...
			float configRate = sWorld.getConfig(CONFIG_FLOAT_RATE_XP_KILL);
			if (configRate != 1.0f)
			{
				uint32 rate = pl->GetXPRate();
				if (rate == 1 || rate == 2)
					return xp_gain*rate;
			}

            return (uint32)(xp_gain*configRate);
        }
