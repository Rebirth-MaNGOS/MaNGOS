...
class PathInfo
{
	...
	/* Checks the intersection between a line and a 2D-polygon. The three corner points of the polygon is given by the first three sets of coordinates followed by the line's starting point and the line's end point.
	   The function will return true if the line intersects the polygon within the triangle. The intersection point will be returned in the *_int variables if supplied.*/
	bool IsLineIntersecting2DPoly( double x[5], double y[5], double z[5], float *x_int = NULL, float *y_int = NULL, float *z_int = NULL );
	
	/* Check if the line between pos1 and pos2 in a map is crossing a pathing barrier. */
	bool IsPathingBarrierIntersectedInMap(const WorldObject *caller, float x1, float y1, float z1, 
