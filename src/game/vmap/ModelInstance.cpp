...
namespace VMAP
{
    ...
    bool ModelInstance::intersectRay(const G3D::Ray& pRay, float& pMaxDist, bool pStopAtFirstHit, bool ignoreM2) const
    {
        if (!iModel)
        {
#ifdef VMAP_DEBUG
        ...
        bool hit = iModel->IntersectRay(modRay, distance, pStopAtFirstHit, ignoreM2);
        if (hit)
        {
            distance *= iScale;
            pMaxDist = distance;
