...
void GameObject::DisarmTrap()
{
	if (this->GetGoType() == GAMEOBJECT_TYPE_TRAP)
	{
        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    {
						int detectSkill = goInfo->trap.stealthAffected?(owner->getLevel() * 5):0;
                        MaNGOS::TrapDetectionCheck u_check(this, detectSkill, radius, true, owner);
                        MaNGOS::UnitSearcher<MaNGOS::TrapDetectionCheck> checker(ok, u_check);
                        Cell::VisitGridObjects(this,checker, radius*2);
                        if (!ok)
                            Cell::VisitWorldObjects(this,checker, radius*2);
                    }
                    else                                    // environmental trap
                    {
                        ...
                        // affect only players
                        Player* p_ok = NULL;
						int detectSkill = goInfo->trap.stealthAffected?((owner == NULL)?goInfo->trap.stealthAffected:(owner->getLevel()*5)):0;
                        MaNGOS::TrapDetectionCheck p_check(this, detectSkill, radius, false, owner);
                        MaNGOS::PlayerSearcher<MaNGOS::TrapDetectionCheck>  checker(p_ok, p_check);
                        Cell::VisitWorldObjects(this,checker, radius*2);
                ...
                if (uint32 max_charges = goInfo->GetCharges())
                {
                    if (m_useTimes >= max_charges)
                    {
                        ...
                        if (GetOwner() != NULL)
						    this->SendGameObjectCustomAnim(this->GetObjectGuid());
                    }
                }
            }
            break;
...
bool GameObject::isVisibleForInState(Player const* u, WorldObject const* viewPoint, bool inVisibleList) const
{
    ...
    if(!u->isGameMaster())
    {
		...
		Unit *owner = GetOwner();
        if(GetGOInfo()->type == GAMEOBJECT_TYPE_TRAP && GetGOInfo()->trap.stealthed && owner != NULL && !owner->IsFriendlyTo(u))
        {
			//This thing's stealth is based on (owner.level) * 5
			//Detection is based on (unit.level) * 5 + stealth detect for phase 1
			//Detect distance is 7 + (detect-stealth)/5

			int stealthValue = (owner->getLevel()) * 5;
			int detectValue = u->getLevel() * 5;

			stealthValue += owner->GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_STEALTH,1);
			detectValue += u->GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_STEALTH_DETECT,1);

			int skillDiff = detectValue - stealthValue;

			visibleDistance = 7.0f + ((float)skillDiff)/5.0f;
        }
...
void GameObject::Use(Unit* user)
{
	...
	if (user->GetTypeId() == TYPEID_UNIT && sScriptMgr.OnMobGameObjectUse((Creature*)user, this))
		return;
...
void GameObject::SetChestRestockTime(time_t time)
{
    m_currentChestRestockTime = time;
}

time_t GameObject::GetChestRestockTime()
{
