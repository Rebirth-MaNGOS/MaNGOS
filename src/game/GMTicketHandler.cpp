...
{
	// If a ticket is protected only a GM can modify it.
	GMTicket* ticket = sTicketMgr.GetGMTicket(GetPlayer()->GetObjectGuid());
	if (ticket && ticket->IsProtected() && !GetPlayer()->isGameMaster())
		return;
...
{
	// If a ticket is protected only a GM can remove it.
	GMTicket* ticket = sTicketMgr.GetGMTicket(GetPlayer()->GetObjectGuid());
	if (ticket && ticket->IsProtected() && !GetPlayer()->isGameMaster())
		return;
...
{
	// If a ticket is protected only a GM can overwrite it.
	GMTicket* ticket = sTicketMgr.GetGMTicket(GetPlayer()->GetObjectGuid());
	if (ticket && ticket->IsProtected() && !GetPlayer()->isGameMaster())
		return;
