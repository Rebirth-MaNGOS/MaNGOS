...
PoolObject* PoolGroup<T>::RollOne(SpawnedPoolData& spawns, uint32 triggerFrom)
{
    ...
    if (!EqualChanced.empty())
    {
		...
		while (!L.empty())
		{
			...
			//old version: doesnt guarantee spawn!
    ...
    if (GameObjectData const* data = sObjectMgr.GetGOData(obj->guid))
    {
        ...
        if (MapPersistentState* dataMapState = mapState.GetMapId() == data->mapid ? &mapState : sMapPersistentStateMgr.GetPersistentState(data->mapid, 0))
        {
            ...
            if (dataMap && dataMap->IsLoaded(data->posX, data->posY))
            {
                ...
                else
                {
                    if (pGameobject->isSpawnedByDefault())
                    {
						...
						//SpawnedPoolData& spawns = mapState.GetSpawnedPoolData();
						//sLog.outError("Pool %u: %u Objects spawned, grid loaded, GUID: %u",poolId,spawns.GetSpawnedObjects(poolId),obj->guid);
            ...
            else if(!instantly)
            {
				...
				//SpawnedPoolData& spawns = mapState.GetSpawnedPoolData();
				//sLog.outError("Pool %u: %u Objects spawned, grid not loaded, GUID: %u",poolId,spawns.GetSpawnedObjects(poolId),obj->guid);
            }
			//else
			//	sLog.outError("Failed to spawn object!");
        }
    }
}
