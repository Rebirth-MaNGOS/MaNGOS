    ...
    if (GetStatus() == STATUS_WAIT_JOIN && GetPlayersSize())
    {
        ...
        else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
        {
...

            {
                ...
                {
		    sWorld.SendWorldText(LANG_BG_STARTED_ANNOUNCE_WORLD, GetName(), 10 * GetBracketId() + GetMinLevel(), GetTypeID() == BATTLEGROUND_AV ? DEFAULT_MAX_LEVEL : std::min((unsigned int) DEFAULT_MAX_LEVEL, 10 * GetBracketId() + GetMinLevel() + 9));
                }
            }
        }
    }
    ...
    for(BattleGroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (team == teamId)
		{
			if(plr->getRace()==RACE_HUMAN)
				plr->GetReputationMgr().ModifyReputation(factionEntry, Reputation*1.1);
			else
...
void BattleGround::AddPlayer(Player *plr)
{
	...
	// Update which pet the warlock is using.
	plr->SetWarlockPetBeforeDeath();
...
void BattleGround::SpawnEvent(uint8 event1, uint8 event2, bool spawn, bool despawn_previous)
{
    ...
    if (spawn)
    {
		...
		{
			// If we aren't despawning the previous creatures we still want to delay their respawn.
			BGCreatures::const_iterator itr = m_EventObjects[MAKE_PAIR32(event1, event2)].creatures.begin();
			for(; itr != m_EventObjects[MAKE_PAIR32(event1, event2)].creatures.end(); ++itr)
			{
				Creature* current_creature = GetBgMap()->GetAnyTypeCreature(*itr);
				if (current_creature)
					current_creature->SetRespawnDelay(10 * RESPAWN_ONE_DAY);
			}
			...
			{
				GameObject* current_gameobject = GetBgMap()->GetGameObject(*itr2);
				if (current_gameobject)
					current_gameobject->SetRespawnTime(10 * RESPAWN_ONE_DAY);
			}
    ...
    BGCreatures::const_iterator itr = m_EventObjects[MAKE_PAIR32(event1, event2)].creatures.begin();
    for(; itr != m_EventObjects[MAKE_PAIR32(event1, event2)].creatures.end(); ++itr)
        SpawnBGCreature(*itr, (spawn) ? RESPAWN_IMMEDIATELY : 10 * RESPAWN_ONE_DAY);
    BGObjects::const_iterator itr2 = m_EventObjects[MAKE_PAIR32(event1, event2)].gameobjects.begin();
    for(; itr2 != m_EventObjects[MAKE_PAIR32(event1, event2)].gameobjects.end(); ++itr2)
        SpawnBGObject(*itr2, (spawn) ? RESPAWN_IMMEDIATELY : 10 * RESPAWN_ONE_DAY);
}
