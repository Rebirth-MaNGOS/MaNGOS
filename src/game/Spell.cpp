...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    else if (target->effectMask != 0)                                     // in 1.12.1 we need explicit miss info
    {
		...
		// Extra check for misses, put in combat on dodge/parry/block and if immune, ignore if its distract
		if ((missInfo == SPELL_MISS_DODGE || missInfo == SPELL_MISS_PARRY || missInfo == SPELL_MISS_BLOCK|| missInfo == SPELL_MISS_IMMUNE || missInfo == SPELL_MISS_IMMUNE2) && (m_spellInfo->Id != 1725))
			if (!unit->isInCombat() && unit->GetTypeId() != TYPEID_PLAYER && ((Creature*)unit)->AI())
				((Creature*)unit)->AI()->AttackedBy(real_caster);
    }
...
void Spell::DoSpellHitOnUnit(Unit *unit, uint32 effectMask, bool isReflected)
{
    ...
    if (realCaster && realCaster != unit)
    {
        ...
        if (!realCaster->IsFriendlyTo(unit))
        {
            ...
            // not break stealth by cast targeting or earthbind totem
